...
code TLG_FORMATS tlg_fmt_table[27] =
  {
    {   1,      1125,   0,   0,    0}, /*  0 */  
    ...
    {4400 / 11, 1125,   0,   1,    0}, /* 12 */ /* will not be referenced */
    {4400 / 11, 1125,   1,   1,    3}, /* 13 */ /* will not be referenced */
    ...
    {4400 / 11, 1125,   0,   1,    0}, /* 22 */ /*  1080sF/30        */
    {4400 / 11, 1125,   1,   1,    3}, /* 23 */ /*  1080sF/29.97     */
    {5280 / 11, 1125,   0,   1,    2}, /* 24 */ /*  1080sF/25        */
    {5500 / 11, 1125,   0,   1,    0}, /* 25 */ /*  1080sF/24        */
    {5500 / 11, 1125,   1,   1,    0}  /* 26 */ /*  1080sF/23.98     */ /* 27 elements */
  };
/* gen_params: */
...
void write_plds(UC ndx)
  ...
  jk = tlg_fmt_table[ndx].sync;
  jk <<= 4;  /* sync bits go to bits 5 and 5 of jk */
  ...
  jk &= 0xFC;                        /* bits 2 and 3 are 0     */
  jk |= itmp.charv[0];               /* two MSbits of SPL - 1  */

  spi_transfer(jk, 8); /* 5 */

  jk = itmp.charv[1];                /* rest of SPL - 1 */
  spi_transfer(jk, 8);               /* 6 */
...
/***/
  ...
  jk &= 0xF8;                   /* zero bits 0, 1 and 2 of jk  */
  jk |= itmp.charv[0];          /* 3 MSbits of LPF - 1 */
  spi_transfer(jk, 8); /* 7 */

  jk = itmp.charv[1];           /* rest of LPF - 1     */
  spi_transfer(jk, 8); /* 8 */
