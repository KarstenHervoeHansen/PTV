...
unsigned char code current_FPGA_sw_version=0x11;
//den nuværende MCU software version (bliver vist i PT5300) 10'ere = version, 1'ere = revision
unsigned char code current_MCU_sw_version=23;
...
// !! OBS !! Alting på  TP3 er forlænget/forkortet til 1 ms 
char tp_select[3] = {pps_i, int_pps, none};
...
unsigned char xdata debug_str[11]; //    "ss,p,l     "
...
*		Revision 8: xx/xx/11: Udgivet som version 2.3						*
	...
	while(1)
	{
		...
		if(ext1_rise_flag==1)
		{
			...
			/*	else if(message_counter==8)
				{
					if(sorted_quality[3]==0)
						sprintf(PT5300_str, "No sig.    ");
					else if(sorted_quality[3]>43)
						sprintf(PT5300_str, "Excel. sig.");
					else if(sorted_quality[3]>39)
						sprintf(PT5300_str, "Good sig.  ");
					else if(sorted_quality[3]>23)
			...
			{
				//memcpy(PT5300_str, status_str, 11);
				sprintf(PT5300_str, "%i        ", (int)main_state);
			}*/

			//sprintf(PT5300_str, "Sig %u    ", (unsigned int)sorted_quality[3]);

			sprintf(phase_str, "%010i", (int)OCXO_phase_diff);
