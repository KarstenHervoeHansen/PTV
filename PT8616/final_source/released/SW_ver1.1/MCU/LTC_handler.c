...
//UTC læst direkte fra GPS chip
struct time xdata UTC_time;				
...
//må kun forskyde i +/- 60 sekunder og timer og +/- 24 timer
...
void LTC_manager()
{
	...
	//sammenlign tider når GPS er tilgængelig (og opdater tider, hvis forskellig)
	if(timepulse_status==1)
	{
		if(memcmp(&int_time, &UTC_time, 7))
			get_UTC_time(&int_time);

		if(memcmp(&int_GPS_tow, &GPS_tow.GPS_tow_long, 4))
...
//29/4/08: Rev 2: Udskriver diverse LTC info
void print_LTC_info()
{
	//UTC tid
	sprintf(debug_buffer, "\n\r UTC T: %02d:%02d:%02d", (int)UTC_time.hour, (int)UTC_time.min, (int)UTC_time.sec);
	SW_sendstr(&debug_buffer, strlen(debug_buffer));

	//internal time
	sprintf(debug_buffer, "\n\r INT T: %02d:%02d:%02d", (int)int_time.hour, (int)int_time.min, (int)int_time.sec);
	SW_sendstr(&debug_buffer, strlen(debug_buffer));
	...
	sprintf(debug_buffer, "\n\rTime: ");
	SW_sendstr(&debug_buffer, strlen(debug_buffer));
	SW_sendstr(&LTC_a_str, strlen(LTC_a_str));
