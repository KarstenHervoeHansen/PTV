...
void LTC_manager()
{
	...
	// 20-01-12 - Rev. 2.6 BS: (Hack) LTC viser nu den interne tid fra GPS chippen selvom der ikke er GPS signal.
	if((timepulse_status==1) || (gps_utc_init_ok==1))
	{
		get_UTC_time(&int_time);
		int_GPS_tow=GPS_tow.GPS_tow_long;
		int_GPS_week = GPS_week;
	}
	else
	{
...
{
	// 20-01-12 - Rev. 2.6 BS: (Hack) Hvis der ikke er GPS signal vises en * til venstre for LTC tidskoden
	//						   som advarsel om at uret ikke er synkroniseret.
	...
	//sprintf(LTC_a_str, "%08d", (int)NTSC_offset.NTSC_offset_int);
