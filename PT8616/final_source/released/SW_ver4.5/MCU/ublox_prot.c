...
// Monitor HW
unsigned int xdata noise;
unsigned int xdata maxnoise;

unsigned long xdata lastUTC;
unsigned int xdata utcOverride;
...
unsigned char xdata ck_a, ck_b;
unsigned int xdata buf_index, str_ptr; // char
unsigned int xdata length;
		...
		case 0x01:
			...
			switch(in_packet.id_num)
			{
				...
				case 0x21:
					...
					// Debug if ( (UTC_time_buffer.min >= 40) && (UTC_time_buffer.min < 50) ) UTC_time_buffer.hour = 21;
...
unsigned char send_packet_ack(PACKET *out_packet)
{
	unsigned char c, d;
	...
	for(c=0; c<5; c++)
	{
		...
		if (d == 1) break;//vent på at der er modtaget en hel pakke, eller på time out
	}
	return(d);
}
...
{
	unsigned long xdata tempUTC = 0;
	
	tempUTC = SecsFrom1900(UTC_time_buffer.year, UTC_time_buffer.month, UTC_time_buffer.day, UTC_time_buffer.hour, UTC_time_buffer.min, UTC_time_buffer.sec, 0);
	if ( ((tempUTC >= lastUTC) && (tempUTC < lastUTC + 10)) || (utcOverride >= 300) )
	{
		utcOverride = 0;
		lastUTC = tempUTC;
...
char setup_ublox()
{
	...
	send_packet_ack(&cfg_prt);	

	//send opsætning for timemode
	send_packet_ack(&timemode);

	send_packet_ack(&tp);
	
	//send_packet(&nav2);
	send_packet_ack(&nav5);
