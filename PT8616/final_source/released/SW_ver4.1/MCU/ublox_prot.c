...
/*
void send_packet_ack(PACKET *out_packet, bit getack)
{
	...
	while(!msg_update_flag && timeout > 1000)
	{
		timeout++;
		packet_process();
	}
...
char setup_ublox()
{
	// The wait for acknowledge is made quick and dirty bacause calling interpret_message() from
	// another subrutine uses 304 bytes of XDATA, which we can't afford. 
	...
	while(!msg_update_flag && timer!=1000)	//vent på at der er modtaget en hel pakke, eller på time out
	{
		timer++;
		packet_process();
	}
