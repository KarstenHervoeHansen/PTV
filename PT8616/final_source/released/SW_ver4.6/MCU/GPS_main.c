...
unsigned char code current_MCU_sw_version=46;
...
*						   NMEA data.															     *
*																									 *
*	Vesion 4.2 : 2013-05-23 BS.																		 *
*				 GPS_Startup improved. Every command sent to the GPS Receiver is now acknowledged.	 *
*				 Main_State GPS_Fail now restarts the the state machine and executes WDT Timeout on  *
*				 GPS Chip.																			 *
*																									 *
*	Vesion 4.4 : 2013-06-21 BS.																		 *
*				 LTC Parameters are saved locally to EEprom 30 seconds after last change.			 *
*				 Theese parameters are loaded in LTC init and normally overridden by the PT5300	     *																									 *
*				 during normal startup.	If MCU resets during runtime theese settings are used and	 *
*				 No time jump should occour.														 *
*				 LTC Handler only loads new time from the GPS if new time is within 10 seconds of	 *
*	             the last timestamp. If time is different for more than 5 minutes the new time is    *
*				 loaded anyway.
*																									 *
*	Vesion 4.5 : 2013-08-28 BS.																		 *
*				 GPS Setup bug fixed. Timer1 reload value changed to 0x95 resulting i a baudrate of	 *
*				 9345.																				 *
*																									 *
*	Vesion 4.6 : 2014-03-07 BS.																		 *
*				 LTC parameters are no longer stored in EEProm. When using dropframe and LTC auto	 *
*				 sync the mainframe went out of sync 31 seconds after the LTC sync occoured.		 *
*																									 *
******************************************************************************************************/
...
void main()
{
	...
	lastUTC = 0;
	utcOverride = 290; // Do not load UTC time to LTC the first 10 seconds after startup.
//	EpromWritePending = -1;
	...
	while(1)
	{
		...
		if(main_state!=next_state)
		{
			switch(next_state)
			{
				case start_up:
					...
					/////////////////////// SETUP OCXO /////////////////////////////
					// 29/04/11 Default værdier læses aldrig mere fra EEPROM, da 
				...
				case ublox_error:
					blinkpat=0xFF;
					send_packet(&cfg_rst);			//Force GPS WDT Timeout.
		...
		switch(main_state)
		{
			...
			default:
				next_state = start_up; //ublox_error;
			break;
		}
...
//			sprintf(debug_str, "%02i,%1i,%1i,%1i,%03i",	(unsigned int)main_state, (unsigned int)timepulse_status, (unsigned int)OCXO_locked_status, (unsigned int)GPS_Recovery_Count, (unsigned int)utcOverride); 
			sprintf(debug_str, "%02i,%1i,%1i,%03i", (unsigned int)main_state, (unsigned int)timepulse_status, (unsigned int)OCXO_locked_status, (unsigned int)utcOverride); 

			LED2=0;

/*	Disabled by BS 2014-03-07 v.4.6
			if (EpromWritePending > 0) EpromWritePending--;
			else if (EpromWritePending == 0) 
			{
				EpromWritePending = -1;
				WriteEEpromData();
			}
