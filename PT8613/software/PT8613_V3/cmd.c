...
/*  Project:  PT8642 DLTPG (PT8613), using 8051F320 Cygnal                 */
/*            processor.                                                   */
/*  File  :   cmd.C                                                        */
...
#define PT8641_COMMAND_PREFIX  'V'    /* First letter of PT8642 command */
                                      /* set */
...
#define SWVERSION              30
#define HWVERSION              00
#define HWINFO                 13 /* This generatoar is called PT8613 */
...
#define   DEL_ADDR           3         /* Delay   MSB...LSB 3 bytes */
#define   INTERF_ADDR        6
...
code struct COMMAND_STRUCT CommandTable[NOOFCOMMANDS] =
  ...
  'V', 'A', /* UnitAddressCommand()          0  */
  'V', 'B', /* WriteUserTextCommand()        1  */
  'V', 'C', /* ReadUserTextCommand()         2  */
  'V', 'D', /* ReadSWVersionCommand()        3  */
  'V', 'E', /* ReadHWVersionCommand()        4  */
  'V', 'F', /* ReadHWInformationCommand()    5  */
  /* Settings functions                         */
  'V', 'G', /* GenSystemCommand()            6  */
  'V', 'H', /* GenPatternCommand()           7  */
  'V', 'I', /* GenAttribCommand()            8  */
  'V', 'J', /* GenDelayCommand()             9  */
  'V', 'K', /* GenAudioClickTimeCommand()    10 */
  'V', 'L', /* GenAudioSignalCommand()       11 */
  'V', 'M', /* GenAudioLevelCommand()        12 */

  'V', 'P', /* GenTextPosCommand()           13 */
  'V', 'Q', /* GenTextScaleCommand()         14 */
  'V', 'R', /* GenTextColorCommand()         15 */
  'V', 'T', /* GenTextMovCommand()           16 */
  'V', 'V', /* GenTextOnOffCommand()         17 */
  'V', 'Z', /* GenTextCommand()              18 */
  'V', 'N', /* JTAGMode()                    19 */
  'V', 'O', /* ManualMode()                  20 */
  'V', 'X'  /* GenInterfaceCommand()         21 */
  };
...
void (*uploader)();      /* pointer to uploader function @0x3C00  */
...
void CommandExecute()
  ...
  switch (i)
    ...
    case 20:
      ...
      break;
    case 21:
      GenInterfaceCommand(INTERF_ADDR);
      break;
...
*             - direct all further communication to 8642 (PT8613) 
*               mounted at XA1,
*            :FACT:V24C:COMM 'VO',1,0,0;
...
*
*            Generator may be 1 or 2.  First 4 bits define
...
*            dat variable.  Subsequent querry ('VO?') will send the 
*            result in dat to the user (as hex value).

*           The syntax is:
*           :FACT:V24C:COMM 'VO',Gen,Addr,0; (0 is dummy data)
*           :FACT:V24C:COMM 'VO?';           (standard querry)
...
/*            To load microprocessor program data sent from Master    */
/*            into microprocessor flasn memory. (VN command)          */
/*                                                                    */
/* ARGUMENTS  None                                                    */
/*                                                                    */
/* RETURNS    Nothing                                                 */
...
/*            to PT8612 and each character sent by PT8613 is sent to  */
/*            the PC.                                                 */
/*                                                                    */
/**********************************************************************/
void JTAGMode(void)
...
/* PURPOSE    To set format for given generator (VG command)             */
/*                                                                       */
/* ARGUMENTS  UI addr                                                    */
/*                                                                       */
/* RETURNS    Nothing                                                    */
...
/*          'VL'<ul>,<ul>                                                */
/*                                                                       */
/*  error = SndULUL(HDTPGUnit[subgen].Address, "VL", (UL)gen, (UL)fmt);  */
/*                                                                       */
...
/* PURPOSE    To set pattern for given generator (VH command)      */
/*                                                                 */
/* ARGUMENTS  None                                                 */
/*                                                                 */
/* RETURNS    Nothing                                              */
...
/*                                                                 */
/*                                                                 */
/*          i.e. generator nr, pattern and attributes are sent     */
/*          (no info about delay)                                  */
...
void GenInterfaceCommand(UI addr)
  {
  UC interf;
  ...
  switch (CmdDelimiter)
    {
    case ';':
      interf = (UC)Parameter[1]; /* get text attributes */
      send((UC)Parameter[0], addr, &interf, 1);
      break;
    default:
      break;
    }
...
/* PURPOSE    To set delay for given generator (VJ command)              */
/*                                                                       */
/* ARGUMENTS  None                                                       */
/*                                                                       */
/* RETURNS    Nothing                                                    */
...
/*            transfer it to the FPGA. (VZ command)                     */
/*                                                                      */
...
/*                                                                 */
/*             :FACT:V24C:ADDR BBU_1;COMM 'VB',2,'000000';         */
/*******************************************************************/
...
/*               :FACT:V24C:ADDR BBU_1;COMM 'VC?',2;                   */
/***********************************************************************/
...
/* Revised:   17.07.2002                                                   */
/*                                                                         */
/*  Function: To return software version (VD command)                      */
/*                                                                         */
/*  Returns:  Nothing                                                      */
/*  Notes:                                                                 */
/*            8051F226 board                                               */
...
/* Revised:    990304, KEn                                                 */
/* Function: To return hardware version (VE command)                       */
/***************************************************************************/
...
/* Revised:    990304, KEn                                                 */
/* Function: To return hardware information (VF command)                   */
/***************************************************************************/
...
/* Revised:   980601, KEn                                                  */
/*                                                                         */
/*  Function: Set/read the unit slave address (VA command)                 */
/*                                                                         */
/*  Remarks:  This function is invoked during FindDynamicOptions procedure */
/*            in Master startup sequence, where master sends VA command    */
...
/*            SndChar(DefaultHDTPG_addr, "VA", conn_addr);                 */
/*                                                                         */
/***************************************************************************/
