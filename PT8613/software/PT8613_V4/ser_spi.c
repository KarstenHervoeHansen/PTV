...
/* use the condition below to allow debugging.  If DefaultHDTPG_addr = 0x13,
   then PT8642 program will work properly without the need to reset the Master
*/
#if 1
#define DefaultHDTPG_addr     (UC)0xF6    /* default SLAVE address for PT8642 */
#else
...
bit  transparent;  /* signals that alternative serial driver is to be used */
                   /* file serial.c                                        */
...
void RS232Init()
  {
  MySlaveAddress   = DefaultHDTPG_addr; /* default SLAVE address for PT8642 */
  CS          = 1;
  RxInput     = 0; 
  RxOutput    = 0;
  RxState     = IDDLESTATE;
...
void RS232break(void) interrupt 4
  ...
  if (transparent)
    {
    serint0(); /* use alternative serial driver */
    }
  else
    {
    while (RI0)
