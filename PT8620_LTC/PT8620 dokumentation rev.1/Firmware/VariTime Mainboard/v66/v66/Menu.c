...
if(GPSGenlockUnit.Present)
	Ptr = &LTC8620_menu[0]; //was LTCNdx = 0 ?
else
    ...
    if (FuncSave)
      ...
      if (UcharVal == ActiveGenlockNo)
        ...
        if (ErrorCode = GenlockUnitDrv(UcharVal, UcharVal1))
	  	...
	  	if ( (UcharVal1==BurstLockPAL)|| (UcharVal1==SyncLock625) || (UcharVal1==SDI625Lock)){
			if(UcharVal==GenlockA)
				//TCGConfig.System[Internal]=0;
			TCGConfig.System[UcharVal]=0;
			
		}
		else{
			if(UcharVal==GenlockA)
				//TCGConfig.System[Internal]=1;
			TCGConfig.System[UcharVal]=1;
			}
		...
		if( (UcharVal==GPSGenlock && TCGConfig.Reference==LTC_GPS_Input) || (UcharVal==GenlockA && TCGConfig.Reference==LTC_VITC_Input) || (UcharVal>GPSGenlock && (TCGConfig.Reference==iPAL || (TCGConfig.Reference==iNTSC) ) )){	


			
			TrxTimeCodeRef(TCGConfig.Reference,TCGConfig.System[UcharVal]);
			}
		

		if ((TCGConfig.Reference==iPAL || TCGConfig.Reference==iNTSC) && UcharVal==GenlockA )
			TrxTimeCodeRef(TCGConfig.Reference,TCGConfig.System[Internal]);
	...
	if (TimeCodeUnit.Present){
      ...
      number = 3;
			  ...
			  if (ConfigNdx < ConfigTimeItem)
				...
				Ptr = &config_menu[ConfigTimeItem];
				start = ConfigTimeItem;
				number = 1;
					}
...
void DiagnoseConfigTest()
  ...
  switch (UcharVal )
	...
	case 33:					// TimeClock
		 ...
		 // KUNumber	= TimeClockUnit.KUNumber;
		  SWVersion = TimeCodeUnit.SWVersion;
		  HWVersion = TimeCodeUnit.HWVersion;
		  HWType	= TimeCodeUnit.HWType;
		  break;
...
void SDITimeAdjust(UC item, UC system, long delay)
  ...
  void TimeDown() {
	...
	WriteCode1(config_menu[ConfigNdx].ItemTxt);
	WriteChar1('/');
	WriteCode1(time_menu[TimeNdx].ItemTxt);
	...
	PrevPtr = Ptr;					// Store old menu-pointer
	Ptr = &time_submenu[TimeNdx];  // Point to selected item
  ...
  void SetRTCTimeLong(UL TB ){
	...
	UC TimeBytes[5];
		  ...
		  if((TCGConfig.System[Internal]))//PAL
		  ctrl1temp=0x88;
		  else
		  ctrl1temp=0x08;

		TimeBytes[0]=(UC) set_hh;
		TimeBytes[1]=(UC) ((TB>>16)&0xFF);
		TimeBytes[2]= (UC) ((TB>>8)&0xFF);
		TimeBytes[3]=(UC) ((TB)&0xFF);
		TimeBytes[4]=ctrl1temp;
		IIC1WriteN(LTC_addr,5,TimeBytes);


		//  IIC1Write2(LTC_addr, (UC) , 0x08);
...
return ((UC2bcd(IIC1RdChar( LTC_addr, get_hh)) << 16)+\
		(UC2bcd(IIC1RdChar( LTC_addr, get_mm))<<8)+\
		(UC2bcd(IIC1RdChar( LTC_addr, get_ss))));
}
...
char * GetTimeCode(){
	...
	TimeBytes[1]=IIC1RdChar( LTC_addr, get_hh);
	TimeBytes[2]=IIC1RdChar( LTC_addr, get_mm);
	TimeBytes[3]=IIC1RdChar( LTC_addr, get_ss);


	//return TimeBytes;
	sprintf(TBuff,"%02d,%02d,%02d",TimeBytes[1],TimeBytes[2],TimeBytes[3]);
				   return TBuff;
   }
	 ...
	 UC sys;
	   ...
	   WriteCodeLN2(0, RefHdrTxt);
	   WriteCode2(TimeRefArrTxt[ TCGConfig.Reference]);
	   ClearRestLN2();
