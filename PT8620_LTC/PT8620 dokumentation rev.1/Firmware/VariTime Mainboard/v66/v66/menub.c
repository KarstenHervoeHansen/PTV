			...
			if(LTCNo==0)
			{
				if(TimeCodeUnit.Present){
					if(GenlockConfig[GPSGenlock].System==5){
						...
						LTCConfig[LTCNo].Format=3; //set to dropframe
						}
					else if(GenlockConfig[GPSGenlock].System==4){
						IIC1Write2(d2ram_addr, 0x0B, 1);
						LTCConfig[LTCNo].Format=1; //set to 25fps/PAL
						}
					}
	...
	if (FuncSave)
	{
		...
		if(FuncState==10)
		{
			...
			if(TimeCodeUnit.Present)
			{
				...
				Ptr=&LTC8620_menu[LTCSelectItem]; // Set pointer to the LTC8620 menu
				//LTCMenuShow();
				ctrl1temp=ctrl1;
				ctrl1=ctrl1|0x04;
				...
				Ptr = &LTC_menu[LTCSelectItem]; // Set pointer to the LTC menu
			Ptr-> DispFunct();               // Display previous menu
			return;
		}
	...
	if (FuncSave || FuncEscape)
	{
		...
		if(FuncState==10)
		{
			...
			if(TimeCodeUnit.Present)
			{
				...
				Ptr = &LTC_menu[LTCSelectItem];
			//return to top menu
			FuncState=10;
			LTCMenuShow();
			// Set pointer to previous menu
			Ptr-> DispFunct();               // Display previous menu
			return;
		}
...
void LTC8620Sync()
{
	...
	case 11:
		WriteItemArrows(20, Cstrlen(LTCSyncTxt));
		WriteArrowsField(L_+U_+R_+D_);
		break;
...
void TimeMenuShow()
{
	...
	WriteCode1(LTCTxt);
	WriteChar1('/');
	WriteCode1(time_menu[TimeRefItem].ItemTxt);
	WriteCode1(CoSelectTxt);
