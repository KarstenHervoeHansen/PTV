...
code char LTCPT8620LevelCmdTxt[] = "PTXXXX";
...
code char MANUALSYNCCmdTxt[]			 = "MANUALSYNC";

code char MANSYNCCmdTxt[]			 = "MANSYNC";
...
//:OUTP
code struct CMD_STRUCT Output[12] =
  {
	...
	{ LTCPT8620LevelCmdTxt,	  LTCPT8620LevelCmdTxt,  NULL,			&LTC_PT8620_Level1[0],		 NONE, LTC}	
};


//:OUTP:LTCG

code struct CMD_STRUCT LTC_PT8620_Level1[6] = 
  {
	  {SYNCCmdTxt,	SYNCCmdTxt, &LTC_PT8620_Level1[1], 	NULL, NONE, LTCFormat},
	  {TIMEParTxt, 	TIMEParTxt, &LTC_PT8620_Level1[2], 		&LTCTimeLevel[0], NONE, LTCTime},
	  {REFERENCECmdTxt,		REFCmdTxt, &LTC_PT8620_Level1[3], 	&LTC_PT8620_Level2[0], NONE, LTC_PT8620_Reference}, // go to next level(LTC_PT8620_Level2)
	  {VERSIONCmdTxt,	VERSCmdTxt,  &LTC_PT8620_Level1[4],	NULL, NONE, LTC_PT8620_ManualSync},
	  {MANUALSYNCCmdTxt,	MANSYNCCmdTxt,  &LTC_PT8620_Level1[5],	NULL, NONE, LTC_PT8620_ManualSync},
	  {DEBUGCmdTxt,		DEBUGCmdTxt, NULL,			NULL, NONE, LTCDebug}
  };



code struct CMD_STRUCT LTC_PT8620_Level2[3] = 
  {
	  {REFERENCECmdTxt,		REFCmdTxt, &LTC_PT8620_Level2[1], 	NULL, NONE, LTC_PT8620_Reference},
	  {TIMEParTxt,	TIMEParTxt,  &LTC_PT8620_Level2[2],	NULL, NONE, LTCVersion},
	  {DEBUGCmdTxt,		DEBUGCmdTxt, NULL,			NULL, NONE, LTCDebug}
  };


code struct PAR_STRUCT TimeCodeRefPar[4] =
{
	{ VITCParTxt, VITCParTxt, &TimeCodeRefPar[1]},
		{PALParTxt, PALParTxt, &TimeCodeRefPar[2]},
			{NTSCParTxt, NTSCParTxt, &TimeCodeRefPar[3]},
			{ GPSTxt, GPSTxt, NULL},
};
		...
		switch (ParameterType)
		{
			...
			case P_String:
				if (TimeCodeUnit.Present)
					...
					//Set the current system to PAL/NTSC
				...
				else if (ParState == 1) //NONE, CONF, AUTO
				{
					ParState++;	
...
/*  
/***************************************************************************/
void LTC_PT8620_RTCTime()
{

	UC tmp;
	char *timecodeptr;
// sprintf(tbuff," parstate=%d",ParState);
	//				 DebugStr(tbuff);
	...
	if (TimeCodeUnit.Present)
	{
		...
		if (CmdExecute && !SCPIError)
		{
			if (CmdRequest)
			{
				...
				{
					timecodeptr=GetTimeCode();
					Udr_PutNChar(timecodeptr);
					Udr_PutNewline();
				}
			}
			else
			{
				...
				else
				{
					...
					if (ValidTimeCode(UlongVal))
					{
						SetRTCTimeLong(UlongVal);
					}
					else
						SCPIError = ExecErr;
				}
	...
	if (TimeCodeUnit.Present)
	{
		...
		case P_Char:
			if (ParState == 0)
			{
				...
				ucharval = FindParameter(Cmd, TimeCodeRefPar);
		...
		if (CmdExecute && !SCPIError)
		{
			if (CmdRequest)
			{
				...
				{
					Udr_CPutNChar(TimeCodeRefPar[TCGConfig.Reference].LongName);
					Udr_PutNewline();
				}
			}
			else
			{
				...
				else
				{
					...
					else if(ucharval==LTC_VITC_Input){
						TCGConfig.System[GenlockA]=GenlockConfig[GenlockA].System;
						tmp=GenlockConfig[GenlockA].System;
						}
					else if(ucharval==iPAL){
						TCGConfig.System[Internal]=0;
				tmp=0;
					}

					else if(ucharval==iNTSC){	
...
void TimeCodeVersion()
{
	...
	if (TimeCodeUnit.Present)
	{
		...
		if (CmdExecute && !SCPIError)
		{
			if (CmdRequest)
			{
				WriteUnitInformation(TimeCodeUnit.HWType, 0);
			}
			else
				SCPIError = SyntaxErr;
		}
