...
void LTC8620RefSelect()
{
	...
	if (FuncSave || FuncEscape)
	{
		if (FuncSave)   			   // If reference has changed..
		{
			...
			sprintf(tbuff,"GenlockConfig[GPSGenlock].System=%2X",GenlockConfig[GPSGenlock].System);
			DebugStr(tbuff);
			...
			//TrxTimeCodeRef(UcharVal,TCGConfig.System[UcharVal]);
...
void RTCTimeSelect()
{
	...
	if (FuncTriggers & 0x30)   		 // If UP/DOWN button pressed..
	{
		...
		TimeFormat(Hour24Format,(UlongVal), 0, TB);
	...
	WriteTxt2(TB);				 // Write edited text on line 2
	ClearRestLN2();				  // Clear rest of line 2
	...
	/*case 4:						// Mark selected item
		...
		break;
*/
	case 1:WriteItemArrows(17, strlen(TB));
	case 4:						// Mark selected item
		UcharVal1 = 0;
		...
		WriteItemArrows(17, strlen(TB));
		CharBlinkLN2(0, OFF);			// Switch OFF blinking
		break;

	case 9:
	case 8:						// Mark selected item
		WriteItemBrackets(17, strlen(TB));
