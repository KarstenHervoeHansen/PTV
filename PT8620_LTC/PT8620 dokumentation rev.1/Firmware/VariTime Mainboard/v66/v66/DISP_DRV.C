...
bit DisplayBusyTest() 
  {
  DispTimer = 10;            // Set  timeout-value at 100ms 10
  BUSY = 1;
  ...
  do
    ...
    if ((ind1 & 0x40) == 0)  // Read & test port, test for Display Data D7
      BUSY = 0;              // at V407 (if D7 is 0, BUSY = 0)
...
void InitDisplay() 
  {
#if 1                  // Original display

  DisplayRS0_RW0;

  waitms(20);          // Delay for >= 15mS after POWER-ON 20
...
/*  Remarks:  Note that there is no test whether the position is legal     */
/*            or not or if the string will exceed the length of line 2.    */
/*          Cursor position on line 2 is updated                           */
/* Returns:   --                                                           */
/* Updates:   --                                                           */
/***************************************************************************/
