...
void FindStaticOptions() {
  ...
  //***********************************************************************
	// Option PT8620 LTC module.
	//***********************************************************************
	//if (IIC1Write2(LTC_addr, 0x03,0x88))//Read reg  



  

if(IIC1RdChar( LTC_addr, 28)==0xAA){
//Check if the LTC module is present. if present it transmits ID=0xAA
	
	  TimeCodeUnit.Present = TRUE;
	  TimeCodeUnit.HWType = PT8620;
	  
	tmpBuffer[1]=IIC1RdChar(LTC_addr,(UC) get_fw_msb);
	tmpBuffer[2]=IIC1RdChar(LTC_addr,(UC) get_fw_lsb);

	
	TimeCodeUnit.SWVersion=tmpBuffer[2]+tmpBuffer[1]*10;
	FeedTheWatchdog;
...
UC find_dl_gen(UC i)
  ...
  /*  							*/
  /***************************************************************************/
  UC TrxTimeCodeRef(UC Ref, UC system) {
...
if (system){
	...
	switch(Ref){
		//LTC_VITC_Input = 0, LTC_GPS_Input, LTC_VideoFieldFreqInput
  		case LTC_VITC_Input: tmp[1]=tmp[1]| 0x20; //VITC
				break;
		case LTC_GPS_Input:
				if (GPS_master_status ==0x03){
					 tmp[1]|= 0x10; //GPS
					TCG_sync_request=1;
					}
				else
		...
		default: tmp[1]=tmp[1]| 0x00; //RTC on VFF
						break;
...
UC GetTCGSystem(){
	UC sys;
	sys=IIC1RdChar(LTC_addr,(UC) ctrl1_reg);
	//sprintf(tbuff,"Ctrl1=**%X**",sys);
	//DebugStr(tbuff);
	sys=BitVal(sys,7);
...
void ChooseTCGRef(UC Ref)
{

		
	switch(Ref){
		case LTC_VITC_Input: 
			TrxTimeCodeRef(Ref,TCGConfig.System[GenlockA]);
			break;
		case LTC_GPS_Input: 
			TrxTimeCodeRef(Ref,TCGConfig.System[GPSGenlock]);
			
			break;
		case iPAL: 
			TrxTimeCodeRef(Ref,0);
			break;
		case iNTSC: 
			TrxTimeCodeRef(Ref,1);
			break;
		}
