...
void HDTPGItemRight()
  ...
  do
    {
    Ptr = Ptr->NextItem;
    } while (!HDTPGUnit[Ptr->ItemNumber].Present);
  }
...
void DLTPGItemRight()
  ...
  do
    {
    Ptr = Ptr->NextItem;
    } while (!DLTPGUnit[Ptr->ItemNumber].Present);
  }
...
void TLGItemLeft()
  ...
  do
    {
    Ptr = Ptr->PrevItem;
    } while (!TLGUnit[Ptr->ItemNumber].Present);
  }
...
void ConfigItemLeft() {
  ...
  if (Ptr->ItemNumber == ConfigDateTimeItem)
	...
	if (Ptr->ItemNumber == ConfigTimeItem)
    if (!TimeCodeUnit.Present)
      Ptr = Ptr->PrevItem;
}
...
void ExecButType6()
  ...
  switch (FuncState)
    {
    case 3:             // if TIMING and EXEC pressed, go to Step choice (10)
      FuncState = 10;
      break;
...
/*  Returns:    --                                                         */
/*  Updates:    VerticalRepeat                                             */
/*  Notes:      This function is called at exit from DLTPGPatternSelect()  */
/*          1                                2                             */
/*      <XXXXXXX>                          <ESC>                           */
/*                                                                         */
/*          3                        4       5                             */
...
/*                                                                         */
/*          9         10            11      12                             */
/*      <WWWWWWW>  <MODIFY>       <SAVE>   <ESC>                           */
/*                                                                         */
/***************************************************************************/
void WriteType7a_SAVE_ESC(char FuncState)
  {

  if (FuncState > 5)
    WriteCodeLN2(19, INTERFACETxt);
  ...
  switch (FuncState)
    ...
    case 7:
    case 10:
      WriteItemArrows(19, Cstrlen(INTERFACETxt));
...
void WriteType7_SAVE_ESC(char FuncState)
  ...
  switch (FuncState)
    ...
    case 7:
    case 10:
      WriteItemArrows(17, Cstrlen(MODIFYTxt));
      WriteArrowsField(L_+D_+R_);
...
/************ NEW **************/
void WriteType7b_SAVE_ESC(char FuncState)
  {

  if (FuncState > 5)
    WriteCodeLN2(17, LevelsTxt);
  ...
  switch (FuncState)
    ...
    case 7:
    case 10:
      WriteItemArrows(17, Cstrlen(LevelsTxt));
      WriteArrowsField(L_+D_+R_);
...
/*******************************  20  **************************************/
/***************************************************************************/
/*  WriteType20_SAVE_ESC                                        MENUHLP.C  */
/*                                                                         */
/* Author:    Janusz Kuzminski, DEV, 31.08.2006                            */
/* Revised:    980827                                                      */
/*                                                                         */
/*  Function:   Handle the EDIT, SAVE and ESC text in the <EDIT SAVE ESC>  */
/*              display handling                                           */
...
/*  Returns:    --                                                         */
/*  Updates:    VerticalRepeat                                             */
/*  Notes:      This function is called at exit from DLTPGSystemSelect()   */
/*          1                                2                             */
/*      <XXXXXXX>                          <ESC>                           */
/*                                                                         */
/*          3                        4       5                             */
...
/*                                                                         */
/***************************************************************************/
void WriteType20_SAVE_ESC(char FuncState)
  {

  if (FuncState > 5)
    WriteCodeLN2(22, ThreeDotsTxt);
...
#if 0
    sprintf(tbuff, "WriteType20_SAVE_ESC: FS = %d\n", (int)FuncState);
    Udr_PutNChar(tbuff);
#endif
  }

/***************************************************************************/
/*  UpButType20                                                  MENUHLP.C  */
/*                                                                         */
/* Author:    Janusz Kuzminski, DEV, 31.08.2006                            */
/* Revised:    31.08.2006                                                  */
/*                                                                         */
...
void LeftButType20()
  ...
  switch (FuncState)
    ...
    case 6:
      FuncState = 13; //8; means that attrib menu is to be shown...
      break;
    ...
    case 9:
      FuncState = 13; //12; means that attrib menu is to be shown...
      break;
...
void RightButType20()
  ...
  switch (FuncState)
    ...
    case 8:
      FuncState = 13; //6; means that attrib menu is to be shown...
      break;
    ...
    case 12:
      FuncState = 13; //9; means that attrib menu is to be shown...
      break;
...
/***************************************************************************/
/* Function below, which have DK_ in their name, are new functions used    */
/* tackle new version of 8603 which has 3 lines of (possibly) movable text.*/
/* New features of 8603 and 5210 are described in specs02.txt              */
/***************************************************************************/
/*  WriteTypeDK_SAVE_ESC                              MENUHLP.C  */
/*                                                  */
/* Author:    Kim Engedahl, DEV, 980302                          */
/* Revised:    980827                                       */
/*                                                  */
...
/*  Remarks:    Part of <XY EDIT SAVE ESC> display handling            */
/*          Cursor postion on line 2 is updated                  */
/*  Returns:    --       WriteType12_SAVE_ESC                                 */
/* Updates:    VerticalRepeat                                */
/***************************************************************************/
void WriteTypeDK_SAVE_ESC(char FuncState)
  {
  WriteAddressLN2(30);

  if ((FuncState >= 3) && (FuncState <= 5) || (FuncState == 8))
    WriteCode2(SAVETxt);
...
/***************************************************************************/
/* DownButTypeDK                                                MENUHLP.C  */
/*                                                                         */
/* Author:    JK                                                           */
/* Revised:   08.10.2004                                                   */
/*                                                                         */
/* Purpose:  To handle DOWN-button in the <XY EDIT SAVE ESC> state machine */
/*           while in SDITextEditSelect() function.                        */
/* Returns:  Nothing                                                       */
/* Updates:  FuncState                                                     */
/* Notes:    Function SDITextEditSelect() displays the following line:     */
/*           LineX: <text_string>     <SAVE>  <ESC>                        */
/*           and awaits user reaction.  Note that, unlike in 8632, there   */
/*           is no <on/off> parameter displayed here.                      */
/*           FuncState has following values:                               */
/*                                                                         */
/*           LineX: <text_string>                    <ESC>                 */
/*                       1                             2                   */
/*           or:                                                           */
/*                                                                         */
/*           LineX: <text_string_change>     <SAVE>  <ESC>                 */
/*                       3                      4      5                   */
/*           and:                                                          */
/*                       7 on executing EDIT  and                          */
/*                       8 on changing text while in EDIT mode             */
/*           Value 6 is not used   .                                       */
/***************************************************************************/
void DownButTypeDK()
  {
...
/***************************************************************************/
/* LeftButTypeDK                                                MENUHLP.C  */
/*                                                                         */
/* Author:    JK                                                           */
/* Revised:   08.10.2004                                                   */
/*                                                                         */
/* Purpose:  To handle LEFT-button in the <XY EDIT SAVE ESC> state machine */
/*           while in SDITextEditSelect() function.                        */
/* Returns:  Nothing                                                       */
/* Updates:  FuncState                                                     */
/* Notes:    Function SDITextEditSelect() displays the following line:     */
...
/***************************************************************************/
/* RightButTypeDK                                               MENUHLP.C  */
/*                                                                         */
/* Author:    JK                                                           */
/* Revised:   08.10.2004                                                   */
/*                                                                         */
/* Purpose:  To handle RIGHT-button in the <XY EDIT SAVE ESC> state        */
/*           machine while in SDITextEditSelect() function.                */
/* Returns:  Nothing                                                       */
/* Updates:  FuncState                                                     */
/* Notes:    Function SDITextEditSelect() displays the following line:     */
/*                                                                         */
...
/***************************************************************************/
/*  WriteTypeTLG_SAVE_ESC                                       MENUHLP.C  */
/*                                                                         */
/* Author:     Kim Engedahl, DEV, 970623                                   */
/* Revised:    980827                                                      */
/*                                                                         */
...
/* Updates:    VerticalRepeat                                              */
/***************************************************************************/
void WriteTypeTLG_SAVE_ESC(char FuncState)
  {
  WriteAddressLN2(30);
...
//    Author: Jens K Hansen
//    
//    Function: Maps FuncStates as (with wrap-around):
//  
...
// Subtracts 10 from state, wrap to 90'ies when below 0'es
void UpButGeneric()
{    
    FuncState -=10;
    
  if (FuncState < 10)
    FuncState = 90+FuncState; 
}

// Adds 10 to State, wrap to 0'es when exceeding 90'es
void DownButGeneric()
{    
    FuncState +=10;
    
  if (FuncState>=90)
    FuncState = FuncState%10;
}
