...
/*                                                                         */
/*  Changes:
/* 08-2013 	 : Added support for TimeCode Module (PT8620)                                                            */
...
#define BitVal(Var, Pos)  ((Var) & (1 << (Pos)))
...
bit resync_is=FALSE;
bit resync_was=FALSE;


bit 	LTCModuleSynced=0;
bit 	VITCNOTOK;
// *************************************************************************
// STATIC VARIABLES
// *************************************************************************
...
//Added TimeCodeUnit 2013, MM, IPU

PNP_STRUCT TimeCodeUnit =
{
	// LTC/VITC  unit (2013)
	FALSE, NA, PT8620, 0, 0, 0, 0, 0
};
...
persistent  TCGMOD_STRUCT TCGConfig;
...
UC TCG_sync_request=0; // 0= no sync needed, 1=sync requested from user, 2=resync (in case of DL,  resync etc) ,  3=synced with GPS
UC TCG_status;
...
void TestGPS_LTC_Resync();

void DebugStr(char *c);



UC ctrl1temp,ctrl1;
UC GPS_LTC_sync(UC system);
UL TimeCodeBuff;
void update();
/***************************************************************************/
/*  waitms                                                          MAS.C  */
	...
	while(TRUE)
	{
		...
		if (GPSGenlockUnit.Present)
		{
			...
			if (Ptr==&LTC_menu[LTCSelectItem])
			{
				...
				{
					/*
					    if (LTC_tbuff2[9]!=LTC_tbuff[9])
					    {
					        //sprintf(tbuff, "%s\n\r",LTC_tbuff );
					        Udr_PutNChar(tbuff);
					        LTC_tbuff2[9]=LTC_tbuff[9];
					    }
			...
			if (slicer==1)
			{
				...
				if((TimeCodeUnit.Present) && ((Ptr==&status_menu[StatusTimeItem] )  ))
				{

					TimeCodeBuff=GetTimeLong();
					TimeFormat(Hour24Format,TimeCodeBuff, 0, LTC_tbuff);

					if(Ptr==&status_menu[StatusTimeItem] )
						WriteTxtLN1(6,LTC_tbuff);
					else
						WriteTxtLN1(27,LTC_tbuff);
				}
					...
					if ((Ptr!=&LTC_menu[LTCSyncConfirmItem]))// && (Ptr!=&LTC_menu[LTCMessageItem]))  //if not already in sync confirm menu
					{
						...
						if (LTC_sync_request&1)  //if request for LTC a resync
						{
							...
							else  //else if automode, do resync
							{
								...
								if(TimeCodeUnit.Present)
								{
									//Ptr=&LTC8620_menu[LTCSelectItem];
									ctrl1temp=ctrl1;
									ctrl1=ctrl1|0x04;
									IIC1Write2(LTC_addr, ctrl1_reg,ctrl1);
									ctrl1=ctrl1temp;
									...
									//********* 2 min loop for at teste resync.
									//		ResyncErrNo++;
									//		TestGPS_LTC_Resync();*/
								}
		...
		if (slicer == 7)
		{
			...
			// *******************************************************************
			// FOR THE PT8620 ONLY: UPDATE STATUS & TIME INFORMATION		WINDOW 7
			// *******************************************************************

			if(TimeCodeUnit.Present) // check if PT8620 is present
			{



				TCG_status=IIC1RdChar( LTC_addr,(UC) ctrl2_reg); // read the ctrl2 register on LTC module

				resync_is=((TCG_status & 4)!=0);


				if (!resync_was && resync_is) { // if we see a positive edge then do manual sync

                             IIC1Write2(d2ram_addr, 52, 1); //send data to GPS module LTC A sync
				...
				resync_was=resync_is; //

				if(GPS_master_status==0x03 && TCGConfig.Reference==LTC_GPS_Input && TCG_sync_request!=2)
				{



					if(TCG_sync_request==0)
					{
						TrxTimeCodeRef(LTC_GPS_Input,TCGConfig.System[GPSGenlock]);

					}

					if(BitVal(TCG_status,7) && ( IIC1ErrorCode == NoIICError))
					{
						if(GPS_LTC_sync(GenlockConfig[GPSGenlock].System))
							TCG_sync_request=2; //clear the sync flag and set synced with GPS
					...
					if(!(VITCNOTOK) && !(BitVal(TCG_status,3)))
					{

						VITCNOTOK=1;
						//TrxTimeCodeRef(2,TCGConfig.System[Internal]);
		...
		if (slicer == 9)
		{
			...
			else
			{
				...
				else if (SendContrast(LCDContrast))       // Update contrast
				...
				*/
				...
				if (GPSGenlockUnit.Present && (ActiveGenlockNo == GPSGenlock))
				{
					...
					if (((syncstatus & 0x01) && (GPS_master_status & 1) && (GPS_master_status & 2)) ^ !UnlockedLED)
					{
						...
						LTCModuleSynced=!LTCModuleSynced;
						UpdateLEDs();
						i = 1;              // Set flag "UPDATE STATUS"
				...
				else if ((syncstatus & 0x01) ^ !UnlockedLED) //only change LED when syncstatus differs from LED status
				{
					UnlockedLED = !UnlockedLED;
					UpdateLEDs();
					i = 1;              // Set flag "UPDATE STATUS"
...
UC GPS_LTC_sync(UC system)
{
	...
	if ( (system==BurstLockPAL)|| (system==SyncLock625) || (system==SDI625Lock))
		ctrl1=0x10;
	else
		ctrl1=0x90;
...
//check if ltc module requests time sync
		LTCByte=IIC1RdChar( LTC_addr, (UC) ctrl2_reg);
#if 0
		if(IIC1ErrorCode==0)
		{
			sprintf(tbuff, "ctrl2=%x read\n\r",LTCByte);
			Udr_PutNChar(tbuff);
		}
		else
		{
		...
		if(BitVal(LTCByte,7) && (IIC1ErrorCode == NoIICError))
		{
			syncerror=0;
#if 0
			sprintf(tbuff, "LTC GPS sync req\n\r",ctrl2_reg);
			Udr_PutNChar(tbuff);
#endif
...
// send gps time to ltc registers
	if(GPSGenlockUnit.Present)
	{
		if(IIC1RdStr(d2ram_addr, 100+LTCNo, LTC_tbuff))
		{
			...
			IIC1WrStrN(LTC_addr,star_reg,LTC_tbuff,12);


		}
	}

//check if ltc module still requests time sync

	LTCByte=IIC1RdChar( LTC_addr, ctrl2_reg);
#if 0
	sprintf(tbuff, "ctrl2=%x read\n\r",LTCByte);
	Udr_PutNChar(tbuff);
#endif
...
void TestGPS_LTC_Resync()
{
	...
	if (LTCConfig[LTCNo].DFMin+2<60)
		LTCConfig[LTCNo].DFMin=LTCConfig[LTCNo].DFMin+2;
	else
	{
		LTCConfig[LTCNo].DFMin=0;
		LTCConfig[LTCNo].DFHour++;
	}
	...
	IIC1Write2(d2ram_addr, 0x0C, LTCConfig[LTCNo].DFMode);
	IIC1Write2(d2ram_addr, 0x0D, LTCConfig[LTCNo].DFHour);
	IIC1Write2(d2ram_addr, 0x0E, LTCConfig[LTCNo].DFMin);
