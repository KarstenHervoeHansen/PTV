...
/* 980616 FreeSpaceInRxBuf() now returns UC instead of char, (problems    */
  ...
  if (rcvChar == LOCK)
    ...
    switch (UART_Handshake)
      ...
      case UDR_NO_HANDSHAKE:          // if no handshake..
        if (Udr_FreeSpaceInRxBuf() > UDR_FLOWSPACE)  // If buffer not full..
          {
          rxBufHead = (++rxBufHead) % UDR_RXBUFSIZE;
          rxBuf[rxBufHead] = rcvChar;      //  store character
          }
        break;
