...
/* External Interrupt 1 (INT1, pin 15), triggered by SPGINT signal */
/* SPGINT signal comes from V309, pin 5.  How it is generated is   */
/* unknown.                                                        */
/* SPGINT = !h10 & !h9 & !h8 & !h7 & !h6;                          */
/*  where hx is address counter for DualPortRam                    */
...
adown:
  atel -= 1;              // count lines
  if (atel == 0)          // taeller rundt til 5 paa line     | disse 3 signaler
    {
    btel -= 1;            // count 3xlines
    if (btel == 0)        // 1 frame gone
      {
      if (freq_sel)
        goto seta;
      if (syncav)
        {
        if (levelcnt)
          levelcnt -= 1;  // good/bad field-filter
        }
      else // NOSNC:
        {
        if (levelcnt & 128)
    ...
    seta:                 // SETA:
      if (g_sel)
        {
        atel  = afield_g; // re-load x-line counter (afield_g = 1)
        btel  = bmax_g;   // re-load 70*9 counter  (209)  // limit for b counter (5-line groups)
        ftel -= 1;
        if (ftel == 0)
          ftel = fmax_g;  // 8 fields gone, re-load frame counter (fmax_g = 4)
        }
      else
        {
        atel  = afield_m; // re-load x-line counter (afield_m = 3)
        btel  = bmax_m;   // re-load 70*9 counter
        ftel -= 1;
        if (ftel == 0)
          ftel= fmax_m;   // re-load frame counter (fmax_m = 2)
        }
    // GETDUR:            // set subcref (correct later for frame no.) *
      if (a10m_sel || genmode == 3)
        ...
        // DPOK:                      // get new duration and line type from table **
        lintyp   = *tableptr;
        tableptr++;
        cvartel  = *tableptr;
        tableptr++;
    ...
    if (atel == acal)
      ...
      if (g_sel)  
        ...
        if (a10m_sel || genmode == 3)
          ...
          subcref += subcinc_g;       // add 25Hz offset to subc
        }
      else
        subcref += subcinc_m;         // add offset to subc
      ...
      // find special lines
      if (freq_sel || d1_sel) 
        doflags |= btable_10[btel];
      else
        if (g_sel)
          doflags |= btable_g[btel];
        else
          doflags |= btable_m[btel];
