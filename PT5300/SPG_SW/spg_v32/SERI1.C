...
********************************************************************
HISTORY:
980915 TI1 is set=0 after S1BUF is loaded (new XA with double buff.)
970213 Serial_end rearranged (mode 1 first)
970114 RELEASED 1.1
...
void put_char1 (UC c)  {
   cksum += c;
   EA= 0; // disable intr.
   ERI1 = 0;  // disable receive
   S1BUF = c;
   TI1= 0;  // clear flag (XA has now double buffering)
   EA= 1; // enable intr.
   while (!TI1);  // wait for transmitter ready
   TI1 = 0;
   ERI1 = 1;  // enable receive
}
// ******************************************************************
...
{  SWR1= 0;     //  SW1 intr request off
   ...
   {  rcksum += rd;
      ...
      if (rd==';'||rd=='?'||rd==0x0A)  // ||rd==0x0D||rd=='$'||rd=='!')
      {  rstate= 2;   // delimiter found, ready for checksum
      }
   }

   else if (rstate==2)    // receive checksum
   {  if (rd== (rcksum & 0x7f))
