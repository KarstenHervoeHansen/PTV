...
PROJ    = io
PROJDIR = c:\cxa\examples\io
PRODDIR = C:\cxa

all         : $(PROJ).abs 

testio.src  : testio.c
	C:\cxa\bin\cxa.exe testio.c -f<<EOF
  $(EMPTY)  -IC:\cxa\include -Ms -So -w -A1 -O1 -g
  EOF

testio.obj  : testio.src
	C:\cxa\bin\asxa.exe testio.src -f<<EOF
  $(EMPTY)  ZP -gs -LcDElMnPQsWXy -OHjs -DMODEL_SMALL CA
  EOF

$(PROJ).out : testio.obj io.i
	C:\cxa\bin\ccxa.exe -Wlk -emEDE=\"$(PROJ).i\" -o $@ -f<<EOF
  $(separate "\n" $(match .obj $!))
  $(separate "\n" $(match .a $!)) -cl    -Wlk-w8 -Ms -LC:\cxa\lib\xa
  EOF

$(PROJ).abs : $(PROJ).out io.i
