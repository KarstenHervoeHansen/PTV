...
architecture Behavioral of genlock_sync is
...
type LockStateType is (
    UnLocked,
    Locking,
    Locked   
  );
signal f4mState :           LockStateType;
...
ReClkProcess: process (zRreset ,Clk ) 
  begin
    if ( zReset = '0') then
      f4m_reclk <= '0';
	 f4m_delay <= '0';
...
f4mCleanProcess: process (zRreset ,Clk ) 
  begin
    if ( zReset = '0') then
      f4m_Clean <= '0';
	 f4mState <= Unlocked;
