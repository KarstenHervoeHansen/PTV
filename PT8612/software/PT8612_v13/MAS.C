...
bit transparent;                // indicates transparent mode.
...
*/
  ...
  while (1)
    {
    IIC1Write(d1ram_addr, 0xff);
    UserTimer = 10;            // 13 org Set timer to 240-260ms 13
    while (UserTimer)           // delay
      FeedTheWatchdog;
    }
    ...
    if (slicer == 15)
      ...
      if (error)                      // If IIC specific error, nothing to do with level error
        ...
        if (i != LevelStatus)
          {
#if 0
  sprintf(tbuff, "WIN15: i=0x%x. LevelStatus = 0x%x\n", (int)i, (int)LevelStatus);
  Udr_PutNChar(tbuff);
#endif
          ...
          if (LevelStatus ^ 0x7FFF)
            ...
            if (ErrorStatus == (SystemErrorType + LevelDetectorError))
              ...
              ErrorStatus = 0;
#if 0
    sprintf(tbuff, "slicer == 15, ErrorStatus = %d\n", ErrorStatus);
    Udr_PutNChar(tbuff);
#endif
              if (!MessageWindowOn && !MenuModeOn)
                Ptr->DispFunct();
...
void transparent_mode(int board)
  {
  FeedTheWatchdog;
#if 0
  sprintf(tbuff, "Transparent, board = %d\n", board);
  Udr_PutNChar(tbuff);
  ...
  switch (board)
    ...
    case 1:
      ...
      MRES2_on;
      MRES3_on;
      SndInt(0x11, "UN", 1);    /* Set JTAG mode */
      break;
    case 2:
      MRES2_off;                // Release reset 2
      MRES1_on;
      MRES3_on;
      SndInt(0x12, "UN", 1);    /* Set JTAG mode */
      break;
    case 3:
      MRES3_off;                // Release reset 3
      MRES2_on;
      MRES1_on;
      SndInt(0x13, "UN", 1);    /* Set JTAG mode */
      break;
    }
