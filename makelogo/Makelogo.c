 ...
 * TEST
 * LAST UPDT:       3.06.2003:
 * NOTES:           This program inserts logos into Phillis 4x4 625 pictures.
 *                  Modification 911.  There are 3 logos: upper, middle and
 *                  lower, to be inserted into upper text field, into 250
 *                  kHz field and into lower text field.  The 250 kHz field
 *                  is filled with black first.  Refer to "Billede til PT8633"
 *                  for picture addresses. Dump.bmp file is produced containing
 *                  the 3 Phillips 4x3 pictures with logos.
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
#define IMHEIGHT 2080          /* height of output image */
...
/*********************************************************/
  ...
  for (i = 0; i < 41; i++)
    {
    fileload((UL)((IMSTART + 413) + i)); /* load lines w/o BGC */
    for (j = 0; j < 70; j++)             /* copy 70 samples to buffers */
      {
      Ybuff[j] = chan1_buff[j];
      Ubuff[j] = chan2_buff[j];
      Vbuff[j] = chan3_buff[j];
      }

    fileload((UL)((IMSTART + 329) + i)); /* load lines with BGC */
    ...
    filesave((UL)((IMSTART + 329) + i));
    }


  for (i = 0; i < 41; i++)
    {
    fileload((UL)((IMSTART + 989) + i)); /* load lines w/o BGC */
    for (j = 0; j < 70; j++)             /* copy 70 samples to buffers */
      {
      Ybuff[j] = chan1_buff[j];
      Ubuff[j] = chan2_buff[j];
    ...
    fileload((UL)((IMSTART + 905) + i)); /* load lines with BGC */
    for (j = 0; j < 70; j++)             /* copy 70 samples from buffers */
      {
      chan1_buff[j] = Ybuff[j];
      chan2_buff[j] = Ubuff[j];
...
/* first copy transit between black and while line from right side and 
   put it mirrored into left side.
   First picture: Philips 4x3 w.antipal 1st part
*/
  start_line   = 369;
  x_offset     = 356;
  x_coordinate = 554;

  for (y_coordinate = 0; y_coordinate < 4; y_coordinate++)
    {
    fileload((UL)(start_line + y_coordinate));
    for (i = 0; i < 20; i++)
      {
      chan1_buff[x_coordinate - x_offset - i] = chan1_buff[x_coordinate + i];
      chan2_buff[x_coordinate - x_offset - i] = chan2_buff[x_coordinate + i];
      chan3_buff[x_coordinate - x_offset - i] = chan3_buff[x_coordinate + i];
      }
    filesave((UL)(start_line + y_coordinate));
    }

/* Now fill 3 lines with black */

  start_line   = 370;

  for (i = 0; i <  CHAN_LINE_LEN * 3; i += 3)
    {
    bmp_buff[i]     = 0x0; /* Black */
    bmp_buff[i + 1] = 0x0;
    bmp_buff[i + 2] = 0x0;
    }
...
/* Now fill rest of lines with black */
...
/* first copy transit between black and while line from right side and 
   put it mirrored into left side.
   Second picture: Philips 4x3 w.antipal 2nd part, offset is 576
*/
  start_line   = 369 + 576;
  x_offset     = 356;
  x_coordinate = 554;
...
/* first copy transit between black and while line from right side and 
   put it mirrored into left side.
   Third picture: Philips 4x3 wo.antipal, offset is 576 + 576
*/
  start_line   = 369 + 576 + 576;
  x_offset     = 356;
  x_coordinate = 554;
...
*/
  open_logo_file("upper.bmp", &bmp_header, &bmp_info);
  ...
  /* Upper logo, Philips w.a. 1st, file: upper.bmp, upper left coordinates are: 304, 293  */
...
/* copy Ybuff....with filtered logo back to chanx_buff */
  ...
  /* Upper logo, Philips w.o antipal, file: upper.bmp, upper left coordinates are: 304, 293 + 576 */
...
/* copy Ybuff....with filtered logo back to chanx_buff */
  ...
  /* Upper logo, Philips w.a. 2nd, file: upper.bmp, upper left coordinates are: 304, 293 + 576 + 576  */

  start_line = 293 + 576 + 576; 
...
#if 1
#undef BMP_FILE
#define BMP_FILE "middle.bmp"  /* File containing middle logo */
#undef NR_LINES
#define NR_LINES          40   /* Number of lines in logo picture  */


  /* Middle logo, file: middle.bmp, upper left coordinates are: 193, 369  */
  ...
  /* The number of bytes in each line of a .BMP file is always
     ...
     how many bytes there are in a line.
  */
  open_logo_file("middle.bmp", &bmp_header, &bmp_info);
  bmp_bytes_per_line = bmp_info.biWidth * 3;
  if (bmp_bytes_per_line & 0x0003)
    {
    bmp_bytes_per_line |= 0x0003;
...
#if 1
#undef BMP_FILE
#define BMP_FILE "lower.bmp"  /* File containing middle logo */
#undef NR_LINES
#define NR_LINES          39   /* Number of lines in logo picture */


  /* Lower logo, file: lower.bmp, upper left coordinates are: 298, 665  */
  ...
  /* The number of bytes in each line of a .BMP file is always
     ...
     how many bytes there are in a line.
  */
  open_logo_file("lower.bmp", &bmp_header, &bmp_info);
  bmp_bytes_per_line = bmp_info.biWidth * 3;
  if (bmp_bytes_per_line & 0x0003)
    {
    bmp_bytes_per_line |= 0x0003;
