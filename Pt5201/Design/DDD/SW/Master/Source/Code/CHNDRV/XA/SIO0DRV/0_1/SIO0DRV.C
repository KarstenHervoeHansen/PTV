...
void sio0_init( void)
{
  ...
  sio0iThread=newTThread( 1, sio0i);
  sio0oThread=newTThread( 2, sio0o);
  ...
  do
  {
    ...
    if ( KS_dequeuet( sio0iStream->ID, &tmp, RxTimeout/CLKTICK) == RC_GOOD)
    {
      switch ( tmp)
        ...
        case 0x81:
          return( sio0TxCksumError);

        default:
          return( sio0UnknownError);
			}
    }
  } while ( retransmit--);

  return( sio0NoAcknowledge);
}										
    ...
    *(--ptr) = 0;			  			// Remove delimiter
    ...
    if ( KS_dequeuet( sio0iStream->ID, &tmp, RxTimeout/CLKTICK) == RC_GOOD)
    {
      if ( tmp - ( checksum & 0x7F))
			  return( sio0RxCksumError);

      return( sio0NoError);
    }
  }
                             
  return( sio0RxTimeout);
}										
...
int sio0SndRequest( int cnt, int addr, char *result, const char *format, ...)
{
  ...
  if (( ret = sio0WaitAnswer( buf)) == sio0NoError)
		strcpy( result, buf);
