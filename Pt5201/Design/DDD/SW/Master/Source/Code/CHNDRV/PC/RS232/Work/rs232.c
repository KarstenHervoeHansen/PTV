...
static char ichar[2]; /* last char read for input */
...
FRAME *int232(FRAME *frame)
{
   ...
   while(IIR.Bits.ID!=0)
   {
      ...
      {
         if (ichar[1]!=ERR_NONE)
            inportb(UART_RBR); /* clear USART interrupt */
         else
         {
            ichar[0] = inportb(UART_RBR); /* read char, clear USART interrupt */
            if (LCR.Bits.DataBits==2)
               ichar[0]&=0x7F;		  /* 7 data bits */
...
//         outportb(PIC_EOI_ADDR, EOI); /* clear PIC interrupt */
      ...
      else if (IIR.Bits.ID == Err)
      {
         ...
         if (LSR.Bits.OverflowErr==1)
            ichar[1]|=ERR_OVERFLOW;
         if (LSR.Bits.ParityErr==1)
            ichar[1]|=ERR_PARITY;
         if (LSR.Bits.FramingErr==1)
            ichar[1]|=ERR_FRAMING;
         if (LSR.Bits.BreakInt==1)
            ichar[1]|=ERR_BREAK;
//         outportb(PIC_EOI_ADDR, EOI); /* clear PIC interrupt */
         KS_ISRsignal(rs232iEvent->ID); /* signal char output semaphore */
      }
      IIR.Byte = inportb(UART_INT_ID_REG); /* read hw status */
