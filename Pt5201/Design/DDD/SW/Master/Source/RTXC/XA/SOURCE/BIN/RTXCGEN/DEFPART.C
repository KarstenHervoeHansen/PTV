...
#ifdef DYNAMIC_PARTS
re_dnpart:
            tmp = getnum("# of dynamic partitions> ");
...
create:
      ...
      case 'G': /* generate new C file */
         ...
         if (konst)
            fprintf(outf,"const ");
	 fprintf(outf,"MAP nparts = NPARTS;\n\n");
#ifdef DYNAMIC_PARTS
         if (konst)
            fprintf(outf,"const ");
	 fprintf(outf,"MAP dnparts = DNPARTS;\n\n");
#endif
         ...
         for (i = 1; i <= nparts; i++)
         {
	       ...
	       fprintf(outf,"static char part%d[M%dCT*M%dSZ];\n",i,i,i);
#else
               if (mem[i].desc[0] != '-')
         ...
         {
#ifdef DYNAMIC_PARTS
	    fprintf(outf,"PHEADER ks_tblx pheader[1+NPARTS+DNPARTS];\n\n");
#else
	    fprintf(outf,"PHEADER ks_tblx pheader[1+NPARTS];\n\n");
#endif
            if (konst)
               fprintf(outf,"const ");
	    fprintf(outf,"PKHEADER pkheader[1+NPARTS] = \n");
         }
         else
#ifdef DYNAMIC_PARTS
	    fprintf(outf,"PHEADER pheader[1+NPARTS+DNPARTS] = \n\n");
#else
	    fprintf(outf,"PHEADER pheader[1+NPARTS] = \n\n");
#endif
