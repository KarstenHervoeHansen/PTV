...
timers:
      ...
      case 'G': /* generate new C file  */
       ...
       fprintf(outf,"/* cclock.c - RTXC Clock definitions - %20s %14s */\n\n",
         ...
         fprintf(outf,"int near ntmrs = NTMRS;\n");
         if (konst)
            fprintf(outf,"extern const ");
         fprintf(outf,"int near clktick = CLKTICK;\n");
         if (konst)
            fprintf(outf,"extern const ");
         fprintf(outf,"int near clkrate = CLKRATE;\n\n");
         fprintf(outf,"CLKBLK near clkq[NTMRS];\n");
         ...
         fprintf(outf,"#define NTMRS %d /* number of timers */\n\n",ntmrs);
         fprintf(outh,"#define CLKRATE %d\n",rate);
         fprintf(outh,"#define CLKTICK %d\n",tick);
         fprintf(outf,"\n/* End of file - cclock.h */\n");
         dirty = 0;    /* clean again */
         fclose(outh);
         break;
