...
swap:
        ...
        case 'G': /* generate new C file */
           if (nsemas == 0)
            ...
            fprintf(outf,"SEMA near nsemas = NSEMAS;\n\n");
            if (konst)
               fprintf(outf,"extern const ");
            fprintf(outf,"int near siglistsize = SIGLISTSIZE;\n\n");
            fprintf(outf,"SEMA near siglist[SIGLISTSIZE];/* sema list */\n\n");
            fprintf(outf,"SSTATE near semat[1+NSEMAS];\n\n");
#ifdef SEMA_USE_TABLE /* { */
