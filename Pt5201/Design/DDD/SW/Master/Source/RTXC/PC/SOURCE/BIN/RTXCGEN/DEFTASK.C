...
#ifdef DYNAMIC_TASKS
re_dntask:
            tmp = getnum("# of dynamic tasks (-1 == 0) > ");
...
create:
        ...
        case 'G': /* generate new C file and H file iff dirty */
            has_fputask = 0;  // initialize boolean to FALSE
#ifdef DYNAMIC_TASKS
            if ((ntasks+dntasks) == 0)
#else
            if (ntasks == 0)
           ...
           fprintf(outf,"/* ctask.c - RTXC Task definitions - %20s %14s */\n\n",
            ...
            fprintf(outf,"TASK near ntasks = NTASKS;\n\n");
#ifdef DYNAMIC_TASKS
            if (konst)
               fprintf(outf,"extern const ");
            fprintf(outf,"TASK near dntasks = DNTASKS;\n\n");
#endif
            for (i = 1; i <= ntasks; i++)
            {
                if (task[i].name[0] != ' ')
                {
                    fprintf(outf,"static int ks_stk stack%d[STKSZ%d/sizeof(int)];\n",i,i);
                }
            }
            ...
            if (konst)
               fprintf(outf,"extern const ");
            fprintf(outf,"TASK near startls[] =\n");
            fprintf(outf,"{\n");
            ...
            for (i = 1; i <= ntasks; i++) /* check for fpu tasks */
            {
                if (task[i].fpumode[0] == 'Y')
                    has_fputask = 1;
            }
            fprintf(outf,"#define ANY_FPU_TASKS %d\n", has_fputask);
            fprintf(outf,"#if !defined(FPU) && ANY_FPU_TASKS\n");
            fprintf(outf,"#error \"Task requiring FPU support is defined, enable FPU support\"\n");
            fprintf(outf,"#endif\n\n");

            for (i = 1; i <= ntasks; i++) /* allocate fpu save areas */
            {
                if (task[i].fpumode[0] == 'Y')
                    fprintf(outf,"static FPREGS near fpu%d;\n",i);
            }
            fprintf(outf,"\n");

            if (konst)
               fprintf(outf,"extern const ");
            fprintf(outf,"KTCB near rtxktcb[1+NTASKS] =\n");
            fprintf(outf,"{\n");
            for (i = 0; i <= ntasks; i++)
            {
                if (task[i].name[0] == ' ')
                ...
                {
#ifdef VARNAMLEN
                    fprintf(outf,"  { %*s, (char *)stack%d, STKSZ%d, %d",
                            namlen,task[i].entry,i,i,task[i].priority);
#else
                    fprintf(outf,"  { %8s, (char *)stack%d, STKSZ%d, %d",
...
#ifdef DYNAMIC_TASKS
            fprintf(outf,"TCB near rtxtcb[1+NTASKS+DNTASKS];\n\n");
#else
            fprintf(outf,"TCB near rtxtcb[1+NTASKS];\n\n");
#endif
