...
/*																									*/
/*	Changes:																						*/
/* 000707:Implemented the checksum calculation.										*/
/*																									*/
/***************************************************************************/
#include <stdlib.h>
...
_rom char SWDateTxt[] 					= "17072000";
_rom char SWVersionTxt[] 				= "02.0";
...
void FlashInfo( void) {
	...
	if ( CmdExecute && !SCPIError) {
		if ( CmdRequest) {
			...
			flash_get_device_info( &manufacturer, &DeviceID);
	...
	if ( CmdExecute && !SCPIError) {
		if ( CmdRequest)
			...
			if ( !ParState)
				...
				if (( tmp=flash_sector_erase_int( byteVal)) != STATUS_READY)
		...
		if ( CmdRequest)
			...
			if ( !ParState)
				...
				for ( i = 0; i < meminfo->nsect; i++) {
					...
					if (( tmp = flash_sector_erase_int( i)) !=  STATUS_READY)
						Udr_CPutNChar( _FL_STATUS[tmp]);
				}
				Udr_CPutNChar( ErasedTxt);
				Udr_CPutNChar( NewlineTxt);
