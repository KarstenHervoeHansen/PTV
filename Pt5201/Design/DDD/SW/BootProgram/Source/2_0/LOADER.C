...
/*																									*/
/* Changes:																						*/
/* 000717: If any error during second writing an error text will be 			*/
					...
					if ( --RecLength) {
						if ( ByteCnt == 2) {
							...
							if (( tmp != STATUS_READY) || ( RecWord != flash_read_word( LoadAddress)))
							{
								...
								if ( RecWord != flash_read_word( LoadAddress))
									Udr_CPutNChar( _FL_WRITEERRORTxt);
							}
