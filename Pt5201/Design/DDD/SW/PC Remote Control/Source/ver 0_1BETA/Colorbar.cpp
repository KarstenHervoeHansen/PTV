...
/*	int i =0;
	...
	if ( CBarSetup->System < NTSC)
	{
		while ( *ptr)
  	{
			if ( PALPatternTable[i++][0])
				PatternComboBox->Items->Add( *ptr++);
    }

	// Setup pattern selection
	PatternComboBox->ItemIndex = PALPatternTable[CBarSetup->Pattern][0];
  }
	else
  {
		while ( *ptr)
  	{
			if ( NTSCPatternTable[i++][0])
				PatternComboBox->Items->Add( *ptr++);
    }

	// Setup pattern selection
	PatternComboBox->ItemIndex = NTSCPatternTable[CBarSetup->Pattern][0];
	}
*/
...
void __fastcall TCBarForm::SystemComboBoxChange(TObject *Sender)
{
  																...
  																OffsetTrackBar))
  {
	  ...
	  if ( CBarSetup->System < NTSC)
		{
			if (( CBarSetup->Pattern == SMPTECBar) || ( CBarSetup->Pattern == FCCCBar))
				CBarSetup->Pattern = EBUCBar;
		}
		else
    {
			if (( CBarSetup->Pattern == EBUCBar) || ( CBarSetup->Pattern == CBarGrey75)\
      	 || ( CBarSetup->Pattern == CBarRed75))
