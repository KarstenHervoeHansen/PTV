...
void __fastcall TPT5201Form::FormCreate(TObject *Sender)
{
  ...
  RestoreWindowPosition( PT5201Form, "");
	...
	// Data-parity-stop e.g. 8-N-2 can NOT be changed
...
void __fastcall TPT5201Form::ConnectConnectItemClick(TObject *Sender)
{
  ...
  StatusBar->Panels->Items[0]->Text = "Connected";
	...
	SendGenlockSystem( GenlockStatus->System);

	CBarStatus = PT5201Setup->GetCBarSettings();
	SendCBarPattern( BlackBurst);
	SendTVSystem( "OUTP:CBG:SYST ", CBarStatus->System);
	SendCBarPattern( CBarStatus->Pattern);
	SendScHPhase( "OUTP:CBG:SCHP ", CBarStatus->ScHPhase);
	SendCBarEmbAudioSignal( CBarStatus->EmbAudio);

 	for ( int i=0; i<3; i++)
		BBStatus[i] = PT5201Setup->GetBlackSettings( i);

	SendTVSystem( "OUTP:BB1:SYST ", BBStatus[0]->System);
	SendTVSystem( "OUTP:BB2:SYST ", BBStatus[1]->System);
	SendTVSystem( "OUTP:BB3:SYST ", BBStatus[2]->System);

	AudioStatus = PT5201Setup->GetAudioSettings();
  SendAudioOutput( AudioStatus->Output);
  SendAudioWordClk( AudioStatus->WordClock);
...
void __fastcall TPT5201Form::SystemStatusUpdate( void)
{
	...
	if ( PT5201CommPort->Connected())
  {
  	...
  	for ( int i=0; i<3; i++)
		...
		CBarPatternEdit->Text = CBarPatternTxt[CBarStatus->Pattern];
...
void __fastcall TPT5201Form::Timer1Timer(TObject *Sender)
{
	if ( PT5201CommPort->Connected() && !TestMode)
  {
		...
		if ( SendRequest( "SYST:ERR?\r\n", 60, buffer))
    {
	    ErrorStatusEdit->Text = buffer;

/*	    if ( SystemReportForm->Visible)
  	  	SystemReportForm->SystemReportMemo->Lines->Add( buffer);*/
    }
    else
    {
...
void __fastcall TPT5201Form::Image2DblClick(TObject *Sender)
{
  ...
  {
  	TestMode = false;
		ShowMessage( "Testmode disabled");
  }
  else
  {
  	TestMode = true;
		ShowMessage( "Testmode enabled");
  }
}
//---------------------------------------------------------------------------
