...
void SetBBDac(unsigned char dac, int BB, int DACType)
  ...
  if (DACType == SYNC_DAC)
    ...
    switch (BB)
      {
      case 1: Fmt(DUTStr,"%s<COMM 52,'HD%i';",dac); // set DAC level
        break;  //BB
      case 2: Fmt(DUTStr,"%s<COMM 52,'JD%i';",dac); 
        break;
      case 3: Fmt(DUTStr,"%s<COMM 52,'KD%i';",dac); 
        break;
      case 4: 
 ...
 if (DACType == OFFSET_DAC){
  switch (BB){
    case 1: Fmt(DUTStr,"%s<COMM 52,'HC%i';",dac); break;
    case 2: Fmt(DUTStr,"%s<COMM 52,'JC%i';",dac); break;
    case 3: Fmt(DUTStr,"%s<COMM 52,'KC%i';",dac); break;
  }  
 }  
...
#endif
/* void StoreBBDac(int BB): Performs FACTORY command ":FACT:BBx:DAC X,Y,Z;" */
/* where X - DACGain   (E)                                                  */
/*       Y - DACOffset (C)                                                  */
/*       Z - DACLevel  (D)                                                  */
...
void StoreBBDac(int BB)
  ...
  Fmt(DUTStr, "%s<:FACT:BB%i:DAC %i,%i,%i;", BB, gaindac, CalBBDac[1], CalBBDac[2]);
  WriteCOMPort(DUTCOM,DUTStr);
...
void SetBBGainDac(unsigned char dac){
 switch (BBNo){
  case 1: Fmt(DUTStr,"%s<COMM 52,'HE%i';",dac); break;
  case 2: Fmt(DUTStr,"%s<COMM 52,'JE%i';",dac); break;
  case 3: Fmt(DUTStr,"%s<COMM 52,'KE%i';",dac); break;
 }
...
void SetPhase(int ph)
  ...
  if (TV == PAL)
    ...
    switch (BBNo)
      {
      case 1: Fmt (DUTStr, "COMM 52,'HK%i';",ph); break;
      case 2: Fmt (DUTStr, "COMM 52,'JK%i';",ph); break;
      case 3: Fmt (DUTStr, "COMM 52,'KK%i';",ph); break;
      } 
    } 
  else
    {
    switch (BBNo)
      {
      case 1: Fmt (DUTStr, "COMM 52,'HM%i';",ph); break;
      case 2: Fmt (DUTStr, "COMM 52,'JM%i';",ph); break;
      case 3: Fmt (DUTStr, "COMM 52,'KM%i';",ph); break;
      } 
    } 
  WriteCOMPort(DUTCOM,DUTStr);
  Delay(0.1);
...
void SetScH(int sch)
  ...
  switch (BBNo)
    {
    case 1: Fmt (DUTStr, "COMM 52,'HH%i';",sch); break;
    case 2: Fmt (DUTStr, "COMM 52,'JH%i';",sch); break;
    case 3: Fmt (DUTStr, "COMM 52,'KH%i';",sch); break;
    } 
  WriteCOMPort(DUTCOM,DUTStr);
  Delay(0.1);
#if DEBUG 
...
int MakeBBPhaseCal(int TVSys) 
 ...
 if (TV == PAL){
  ...
  WriteCOMPort(DUTCOM,":INP:GENL:SYST PALB;");
   Delay(0.01);
    switch (BBNo){
     case 1:  WriteCOMPort(DUTCOM,":FACT:V24C:COMM 52,'HK0';");Delay(0.01);// h-phase
          ...
          break;
     case 2:  WriteCOMPort(DUTCOM,":FACT:V24C:COMM 52,'JK0';");Delay(0.01);// h-phase
          ...
          break;
     case 3:  WriteCOMPort(DUTCOM,":FACT:V24C:COMM 52,'KK0';");Delay(0.01);// h-phase
...
int ga[2];
 ...
 if (TV == PAL){
   ...
   WriteCOMPort(DUTCOM,"DEL 0,0,0;");
   Delay(0.05);
   WriteCOMPort(DUTCOM,":OUTP:TSG:SYST NTSC;PATT CBSMPTE;");
   Delay(0.05);
 ...
 if (PhaseScHGainOk == FALSE)
   switch (TV)
     {
     case PAL:
       MessagePopup (" PAL: Phase, ScH og gain kalibrering", " FEJLET"); 
       PhaseScHGainOk == FALSE;
       break;
     case NTSC:
       MessagePopup (" NTSC: Phase, ScH og gain kalibrering", " FEJLET");
       PhaseScHGainOk == FALSE;
       break;
     }
#endif  
...
int handle_returned, controlID_returned;
 ...
 while (done == FALSE){
  GetUserEvent (NOWAIT, &handle_returned, &controlID_returned);
  WriteCOMPort(DUTCOM,"SYST PAL; PATT CBEB;");
  Delay(PatternShiftDelay);
  GetUserEvent (NOWAIT, &handle_returned, &controlID_returned);
  if (done == TRUE) break;
  WriteCOMPort(DUTCOM,"SYST PAL; PATT CBGREY75;");
  Delay(PatternShiftDelay);
  GetUserEvent (NOWAIT, &handle_returned, &controlID_returned);
  if (done == TRUE) break;
  ...
  GetUserEvent (NOWAIT, &handle_returned, &controlID_returned);
  if (done == TRUE) break;
  WriteCOMPort(DUTCOM,"SYST NTSC; PATT CBSM;");
  Delay(PatternShiftDelay);
  GetUserEvent (NOWAIT, &handle_returned, &controlID_returned);
  if (done == TRUE) break;
  ...
  GetUserEvent (NOWAIT, &handle_returned, &controlID_returned);
  if (done == TRUE) break;
  WriteCOMPort(DUTCOM,"SYST NTSC; PATT PLUG;");
  Delay(PatternShiftDelay);
 } // while
...
char *usertxt;
 ...
 WriteCOMPort(DUTCOM,":OUTP:TSG:SYST PAL; PATT CBEU;"); 
...
// check SDI output  
 ...
 WriteCOMPort(DUTCOM,":OUTP:TSG:SYST PAL; PATT CBEB;");
