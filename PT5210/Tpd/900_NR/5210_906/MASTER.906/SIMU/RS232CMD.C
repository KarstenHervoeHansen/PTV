...
/*																									*/
/*	Changes:																						*/
/* 980113: /906 version. In SDIAudioSignal() only Off and Stereo 1kHz are	*/
/*				allowed.																			*/
/* 971217 /906 version. SDIPattern & SDITextInsert now supports PT8603/905	*/
/*			  and differentiates between standard PT8603 and PT8603/905			*/
/* 971215 /906 version. Altered call to CheckTextASCII. SDITextInsert now	*/
/*			  able to handle 16 characters												*/
...
code char Test1ParTxt[] = "TEST1";
code char Test2ParTxt[] = "TEST2";
code char Test3ParTxt[] = "TEST3";
code char Test4ParTxt[] = "TEST4";
...
code struct PAR_STRUCT SDIPatternPar[21] = {
	...
	{ Test1ParTxt, Test1ParTxt, &SDIPatternPar[18]},
	{ Test2ParTxt, Test2ParTxt, &SDIPatternPar[19]},
	{ Test3ParTxt, Test3ParTxt, &SDIPatternPar[20]},
	{ Test4ParTxt, Test4ParTxt, NULL}
};
		...
		if ( CmdExecute && !SCPIError) {
			if ( CmdRequest) {
				...
				if ( ParState == 1) {
					...
					for ( i=0; i < strlen( Cmd); i++)
						if ( !CheckTextASCII( 2, Cmd[i]))
							break;

					if ( i == strlen( Cmd)) {
						strcpy( SDIConfig.TextInfo, Cmd);
						TrxSDIText( SDIConfig.TextOn, SDIConfig.TextInfo);
					}
					else
						SCPIError = ParErr;
