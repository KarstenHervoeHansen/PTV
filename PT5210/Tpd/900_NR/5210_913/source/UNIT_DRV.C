...
/*		UC AES_EBUUnitDrv( UC ndx, UC Signal, UC Level, UC Timing)				*/
...
/*																									*/
/*	Changes:																						*/
/* 991110: Error In TestSDI/AnalogSignal when POS2 selected corrected		*/
/* 991008: Error in TestAES_EBUUnit() corrected										*/
...
/* 020701  ConfigureSDISignalUnit() modified to accept all 8603 JK         */
/***************************************************************************/
...
#if 0			// Test if the board has been tampered with // JK
...
UC AES_EBUUnitDrv( UC ndx, UC Signal, UC Level, UC Timing) {
	...
	port = AES_EBUSignalTable[Signal] + 0xF880;

	if ( Signal != AES_EBUWordclock)
		port |= AES_EBULevelTable[Level] + AES_EBUTimingTable[Timing];
	...
	if ( !IIC2_Write( addr1, ( UC) port))
		error = IIC2ErrorCode;

	if( !IIC2_Write( addr2, ( UC)( port >> 8)))
		error = IIC2ErrorCode;

	return( error ? ( AES_EBUErrorType + error) : 0);
}
...
UC TrxAnalogText( UC state, char* text) {
	...
	error = SndInt2( AnalogSignalUnit.Address, "AF",\
											 (int) ( state > 0), ( state == TextPos2));
	if ( !error)
...
UC TrxSDIText( UC state, char* text) {
	...
	error = SndInt2( SDISignalUnit.Address, "SF",\
											 (int) ( state > 0), ( state == TextPos2));
	...
	if ( system == SDI625) {							// If system is SDII625..
		...
		if ( SDIConfig.Delay >= ( long) 525*1716*256)	// If delay is invalid..
			SDIConfig.Delay = 0;							// ..reset the delay
																// If pattern is invalid..
		if (( tmp > SDISMPTECBar) && ( tmp < SDICheck))
...
UC ConfigurePreset( UC No) {
	...
	for ( i = 0; i < NoOfAES_EBUUnits; i++)
		memcpy( &AES_EBUConfig[i], &Preset[No].AES_EBUStore[i], sizeof( AES_EBUConfig[0]));
	...
	if ( AES_EBUUnit.Present) {
		for ( i = AES_EBU1Item; i <= last; i++)
			if ( error = AES_EBUUnitDrv( i, AES_EBUConfig[i].Signal, AES_EBUConfig[i].Level, AES_EBUConfig[i].Timing)) {
				ErrorTmp = error;
				ErrorCnt++;
			}
	}
...
UC TestSPGUnit( void) {
	...
	tmp = (UC) ( IIC2_RdLong( spgram_addr, HPHZero_G_addr) - RecLong( sync_addr, "GA", 0xFF, &error));
	if ( error || tmp)
		return( 1);

	tmp = (UC) ( IIC2_RdInt( spgram_addr, GenlPhase_G_addr) - RecInt( sync_addr, "GF", 0xFF, &error));
	if ( error || tmp)
		return( 1);

	tmp = (UC) ( IIC2_RdLong( spgram_addr, HPHZero_M_addr) - RecLong( sync_addr, "GU", 0xFF, &error));
	if ( error || tmp)
		return( 1);

	tmp = (UC) ( IIC2_RdInt( spgram_addr, GenlPhase_M_addr) - RecInt( sync_addr, "GG", 0xFF, &error));
	if ( error || tmp)
		return( 1);
...
UC TestAES_EBUUnit( void) {
	...
	for ( i = 0; i < 2; i++) {
		tmp = AES_EBUConfig[i].Signal;

		port[i] = AES_EBUSignalTable[tmp] + 0xF880;

		if ( tmp != AES_EBUWordclock)
			port[i] |= AES_EBULevelTable[AES_EBUConfig[i].Level] +
						AES_EBUTimingTable[AES_EBUConfig[i].Timing];
	}			
											// Compare level/signal for AES_EBU1
	if ((( port[0] & 0x00FF) - IIC2_Read( sound1_addr)) & 0x7F)
		return( 1);
											// Compare timing for AES_EBU1
	if ( !IIC2ErrorCode)
		if ((( port[0] >> 8) - IIC2_Read( sound2_addr)) & 0x07)
			return( 1);

	if ( Connectors[AESConn].HWType == PT8635) {
											// Compare level/signal for AES_EBU2
		if ( !IIC2ErrorCode)
			if ((( port[1] & 0x00FF) - IIC2_Read( sound3_addr)) & 0x7F)
				return( 1);
											// Compare timing for AES_EBU2
			if ( !IIC2ErrorCode)
				if ((( port[1] >> 8) - IIC2_Read( sound4_addr)) & 0x07)
					return( 1);
		}
...
UC TestAnlBlkUnit( UC conn) {
	...
	if (( fabs( ScHPhase - (float) AnlBlkConfig[no].ScHPhase) > 0.5) || error)
		return( 1);
	...
	if (( fabs( ScHPhase - (float) AnlBlkConfig[no+1].ScHPhase) > 0.5) || error)
		return( 1);

												// Test system in BBn
												// Not implemented in ver 2.2 and earlier
	tmp = AnlBlkConfig[no].System - (UC) RecInt( addr, "HQ", 0xFF, &error);
	if ( tmp || error)
		if ( error != RecV24Timeout)
			return( 1);
												// Test system in BBn+1
												// Not implemented in ver 2.2 and earlier
	tmp = AnlBlkConfig[no+1].System - (UC) RecInt( addr, "JQ", 0xFF, &error);
	if ( tmp || error)
		if ( error != RecV24Timeout)
			return( 1);
												// Test delay in BBn
												// Not implemented in ver 2.2 and earlier
	tmp = (UC) ( AnlBlkConfig[no].Delay - RecLong( addr, "HB", 0xFF, &error));
	if ( tmp || error)
		if ( error != RecV24Timeout)
			return( 1);
												// Test delay in BBn+1
												// Not implemented in ver 2.2 and earlier
	tmp = (UC) ( AnlBlkConfig[no+1].Delay - RecLong( addr, "JB", 0xFF, &error));
	if ( tmp || error)
		if ( error != RecV24Timeout)
			return( 1);
...
UC TestSDIBlkUnit( UC conn) {
	...
	if ( tmp || error)
		return( 1);
												...
												// Test pattern
	tmp = SDIBlkConfig[no].Pattern - (UC) RecInt( addr, "HP", 0xFF, &error);
	if ( tmp || error)
		return( 1);

												// Test EDH insertion
	tmp = SDIBlkConfig[no].EDH - (UC) RecInt( addr, "HE", 0xFF, &error);
	if ( tmp || error)
		return( 1);

												// Test embedded audio
	tmp = SDIBlkConfig[no].EMBAudio - (UC) RecInt( addr, "HU", 0xFF, &error);
	if ( tmp || error)
		return( 1);

												// Test delay
	tmp = (UC) ( SDIBlkConfig[no].Delay - RecLong( addr, "HB", 0xFF, &error));
	if ( tmp || error)
		return( 1);
...
UC TestAnalogSignalUnit( void) {
	...
	float ScHPhase;
												...
												// Examine status byte
	tmp = (UC) RecInt( AnlSig_addr, "AS", 0xFF, &error);
	if ( tmp || error)
		return( 1);

												// Test system
	tmp = AnalogConfig.System - ( UC) RecInt( AnlSig_addr, "AQ", 0xFF, &error);
	if ( tmp || error)
		return( 1);

												// Test pattern
	tmp = AnalogConfig.Pattern - (UC) RecInt( AnlSig_addr, "AP", 0xFF, &error);
	if ( tmp || error)
		return( 1);

												// Test text enable
	tmp = ( AnalogConfig.TextOn > 0);
	tmp -= ( UC) RecInt( AnlSig_addr, "AF", 0xFF, &error);
	if ( tmp || error)
		return( 1);

												// Test text information
	tmp = strcmp( AnalogConfig.TextInfo, RecStr( AnlSig_addr, "AN", 0xFF, &error));
	if ( tmp || error)
		return( 1);

												// Test ScHPhase
	ScHPhase = ( RecInt( AnlSig_addr, "AH", 0xFF, &error) * 360.)/2048.;

	( ScHPhase > 180.) ? ( ScHPhase -= 360.) : 0;

	if (( abs( ScHPhase - (float) AnalogConfig.ScHPhase) > 0.5) || error)
		return( 1);

												// Test delay
	tmp = (UC) ( AnalogConfig.Delay - RecLong( AnlSig_addr, "AB", 0xFF, &error));
	if ( tmp || error)
		return( 1);
...
UC TestSDISignalUnit( void) {
	...
	UC tmp;
												...
												// Examine status byte
	tmp = (UC) RecInt( SDISig_addr, "SS", 0xFF, &error);
	if ( tmp || error)
		return( 1);

												// Test system
	tmp = SDIConfig.System - (UC) RecInt( SDISig_addr, "SQ", 0xFF, &error);
	if ( tmp || error)
		return( 1);

												// Test pattern
	tmp = SDIConfig.Pattern - (UC) RecInt( SDISig_addr, "SP", 0xFF, &error);
	if ( tmp || error)
		return( 1);

												// Test text enable
	tmp = ( SDIConfig.TextOn > 0);
	tmp -= (UC) RecInt( SDISig_addr, "SF", 0xFF, &error);
	if ( tmp || error)
		return( 1);

												// Test text information
	tmp = strcmp( SDIConfig.TextInfo, RecStr( SDISig_addr, "SN", 0xFF, &error));
	if ( tmp || error)
		return( 1);

												// Test EDH insertion
	tmp = SDIConfig.EDH - (UC) RecInt( SDISig_addr, "SE", 0xFF, &error);
	if ( tmp || error)
		return( 1);

												// Test audio signal
	tmp = SDIConfig.AudioSignal - (UC) RecInt( SDISig_addr, "SU", 0xFF, &error);
	if ( tmp || error)
		return( 1);

												// Test audio level
	tmp = SDIConfig.AudioLevel - (UC) RecInt( SDISig_addr, "SL", 0xFF, &error);
	if ( tmp || error)
		return( 1);

												// Test delay
	tmp = (UC) ( SDIConfig.Delay - RecLong( SDISig_addr, "SB", 0xFF, &error));
	if ( tmp || error)
		return( 1);
