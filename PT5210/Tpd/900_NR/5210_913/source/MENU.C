...
/* 990614: Corrected header text in ConfigLockSelect                */
/* 990613: Added new parameters for text position in SDI/Analog text      */
/* 981001: Added support for Staircase 5-/10-step in AnalogPatternSelect() */
/*        for PT8601 sw. 2.5+ AND in SDIPatternSelect() for PT8603 sw.  */
/*        3.5+.                                        */
/* 980915: Added in StatusGenlockShow(): New status for burstlock.      */
/* 980616: Added support for -15dB level in PT8635.                */
/* 980616: Added PT8635 signature in DiagnoseOptionsTest()            */
/*        In StatusErrorUpdate(): Now differentiate between active/no    */
/*        error detected.                                */  
/* 980611: Added PT8610 signature in DiagnoseConfigTest()            */
/* 980609: Corrected GenlockTimeSelect() & ConfigLCDContSelect()        */
/*        PresetRecallSelect()                              */
/*        In DiagnoseMemorytest(): RAMCheckOK() replaced by Diagnose-    */
/*        RAMCheckOK();                                  */
/* 980427: Change in DiagnoseKeyboard(): Whenever the user press a button  */
/*        the test of the button is immediately ended, whether it is the  */
/*        right button or not                              */
/*        Error in ConfigDown(): SetlectModeOn was in some cases set too  */
/*        early                                        */
...
/* 020627: SDITextSelect() modified for version with moving text: JK       */
/***************************************************************************/
...
near UC AES_EBUnNdx;                //     -"-     AES-EBU out item
near UC AES_EBUNdx[NoOfAES_EBUUnits];  //     -"-     AES-EBU out item
  ...
  if ( !Flags.PanelLockOn) {            // If PANEL LOCK is disabled
    ...
    if ( AnalogSignalUnit.Present || SDISignalUnit.Present ||
      ...
      switch ( ActiveOutputNo) {
        ...
        case AES_EBUItem:
          if (( AES_EBUnNdx == AES_EBU1Item) && ( Connectors[AESConn].HWType == PT8635))
            AES_EBUnNdx = AES_EBU2Item;
          else
            if ( AnalogSignalUnit.Present)
              ActiveOutputNo = AnalogSignalItem;
          else
      ...
      switch ( BaseNdx = ActiveOutputNo) {
        ...
        case AES_EBUItem:              // AES-EBU
                                // Select AES-EBU SIGNAL
          AES_EBUNdx[AES_EBUnNdx] = AES_EBUSignalItem;
          Ptr = &AES_EBU_menu[AES_EBUNdx[AES_EBUnNdx]];  //  SUBMENU
...
void StatusAudioUpdate() {
  ...
  UC ndx = Ptr->ItemNumber - StatusAudio1Item;;

  WriteCodeLN1( 0, AES_EBUHdrArrTxt[ndx]);
  WriteCode1( AES_EBUSignalArrTxt[AES_EBUConfig[ndx].Signal]);
  ClearRestLN1();

  WriteCodeLN2( 0, LevelHdrTxt);

  if ( AES_EBUConfig[ndx].Signal == AES_EBUWordclock) {
    ...
    WriteCode2( AES_EBULevelArrTxt[AES_EBUConfig[ndx].Level]);

    ClearToPosLN2( 17);

    WriteCode2( TimingHdrTxt);
    WriteCode2( AES_EBUTimingArrTxt[AES_EBUConfig[ndx].Timing]);
    }
...
void BaseDown(){
  ...
  switch ( BaseNdx) {
    ...
    case AES_EBUItem:                // If AES-EBU MENU selected..
      Ptr = &AES_EBUn_menu[AES_EBUnNdx];  //  update menu-pointer to last
      break;                    //  active item
  ...
  if ( FuncTriggers & 0x03) {          // If UP/DOWN button pressed..
    ...
    if ( UcharVal1 < NTSC) {          // If system is PAL..
      if ( FuncTriggers & 0x01) {      // If button pressed is UP..
        ...
        if ( UcharVal == ModulatedStair5)
          UcharVal--;

        if ( UcharVal == ModulatedRamp)
          UcharVal = BlackBurst;
        }
      else {                    // .. button pressed is DOWN
...
void AnalogTextSelect(){
  ...
  if ( FuncTriggers & 0x03) {          // If UP/DOWN button pressed..
    ...
    TextBuffer[UcharVal] =\
        FindNextASCII(( FuncTriggers & 0x01), TRUE, TextBuffer[UcharVal]);
  ...
  if ( FuncTriggers & 0x0C) {          // If UP/DOWN selected..
    if ( Connectors[BB78Conn].SWVersion >= 28) {
      if ( FuncTriggers & 0x04) {      // If button pressed is UP..
        if ( UcharVal1-- == TextOff)
          UcharVal1 = TextPos2;
        }
      else {                    // ..button pressed is DOWN
        if ( UcharVal1++ == TextPos2)
          UcharVal1 = TextOff;
        }
      }
    else {
  ...
  if ( Connectors[BB78Conn].SWVersion < 28)
    ...
    CodePtr = TextOffOnArrTxt[ UcharVal1];  // Get text for the on/off selection
...
void SDIPatternSelect(){
  ...
  if ( FuncTriggers & 0x03) {          // If UP/DOWN button pressed..
    ...
    if (( Connectors[SDISConn].SWVersion < 35) || ( Connectors[SDISConn].HWType == PT8602)) {
    ...
    else {                      // UP/DOWN for PT8633 SW3.0+.
      if ( UcharVal1 == SDI625) {      // If system is SDI625..
        if ( FuncTriggers & 0x01) {    // If button pressed is UP..
          if ( UcharVal-- == SDIEBUCBar)
            UcharVal = SDIStair10;

          if ( UcharVal == SDIValidRamp)
            UcharVal = SDIRed75;
          }
        else {                  // ..button pressed is DOWN
...
void SDITextSelect(){
  ...
  if ( FuncSave || FuncEscape) {
    if ( FuncSave) {                 // If text has changed..
      ...
      SDIConfig.TextOn = UcharVal1;      // Save text-insertion info
  ...
  if ( FuncTriggers & 0x0C)
    {          // If UP/DOWN pressed in On/Off..
    if (0)     // orig: Connectors[SDISConn].SWVersion >= 37) JK
      {
      ...
      {
      if ( UcharVal1 == ON)               //  toggle selection
        UcharVal1 = OFF;
      else
        UcharVal1 = ON;
      }
  ...
  if (0)  // orig: Connectors[SDISConn].SWVersion >= 37)
    CodePtr = TextOffOnArrTxt[UcharVal1];  // Get text for the on/off selection
  else
    CodePtr = OffOnArrTxt[UcharVal1];    // Get text for the on/off selection
  ...
  switch ( FuncState){
    ...
    case 4:                                      // Mark selected item
      WriteItemArrows( 11, strlen( TextBuffer));
      CharBlinkLN2( 0, OFF);              // Switch OFF blinking
      break;
    ...
    case 9:                                     // Mark selected item
      WriteItemBrackets( 11, strlen( TextBuffer));
      CharBlinkLN2( 11+UcharVal, ON);        // Switch on blinking at
      break;                        //  editing character
    }
...
void SDIAudioSignalSelect(){
  ...
  CodePtr = AudioSignalArrTxt[UcharVal];    // Get text for selected audio signal
...
void SDIAudioLevelSelect(){
  ...
  CodePtr = AudioLevelArrTxt[UcharVal];    // Get text selection audio level
...
/***************************************************************************/
/*  AES_EBUnMenuShow                                    MENU.C  */
/*                                                  */
/* Author:    Kim Engedahl, DEV, 991007                          */
/* Revised:    991007                                       */
/*                                                  */
/*  Function:  Display the items in the AES-EBUn MENU                */
/*  Remarks:    AES-EBUn MENU function                          */
/*          Cursor position on line 1 and line 2 are updated        */
/*          Cursor position on line 2 is updated                */
/*  Returns:    Nothing                                    */
/*  Updates:    Ptr, UcharVal, FuncSave, FunState and FuncTriggers        */
/***************************************************************************/
void AES_EBUnMenuShow(){
  ...
  AES_EBUnNdx = Ptr->ItemNumber;        // Update AES-EBU unit index, ie. no.

  WriteCodeLN2( 0, SubmnuHdrTxt);        // Write header for menu line

  if ( Connectors[AESConn].HWType == PT8635)// If PT8635..
    last = AES_EBU2Item;                  //  two audio generators
  else                          // ..else
    last = AES_EBU1Item;              //  only one audio generator

  for ( i = AES_EBU1Item; i <= last; i++) {
    ClearToPosLN2( AES_EBUn_menu[i].ItemPos);  // Find item column position
    WriteCode2( AES_EBUn_menu[i].ItemTxt);    // Write item texts
    }
  ClearRestLN2();                  // Clear rest of line 2
...
/* Revised:    991007                                       */
/*                                                  */
/*  Function:  Switch to the selected item in the AES-EBUn MENU        */
/*  Remarks:    AES-EBUn MENU function                          */
/*  Returns:    Nothing                                    */
/*  Updates:    Ptr                                      */
/***************************************************************************/
void AES_EBUnDown(){
  ...
  Ptr = &AES_EBU_menu[AES_EBUNdx[AES_EBUnNdx]];
  }
...
/* Revised:    991007                                       */
/*                                                  */
/*  Function:  Select the AES-EBUn MENU and restore the last selected item  */
/*  Remarks:    AES-EBUn MENU function                          */
/*  Returns:    Nothing                                    */
/*  Updates:    Ptr                                      */
/***************************************************************************/
...
void AES_EBUMenuShow(){
  ...
  WriteCode1( AES_EBUn_menu[AES_EBUnNdx].ItemTxt);

  WriteCode1( Select_Txt);            // Write sub-menu headline
  ClearRestLN1();
                              // Update AES-EBU Menu index
  AES_EBUNdx[AES_EBUnNdx] = Ptr->ItemNumber;
  ...
  if ( AES_EBUNdx[AES_EBUnNdx] > AES_EBUSignalItem) {
    if ( AES_EBUConfig[AES_EBUnNdx].Signal != AES_EBUWordclock)
      i += D_;
    }
                      // If NORMAL LOCK enabled..
                      //  include a pad-lock and remove the
...
/* Revised:    991110                                       */
/*                                                  */
/*  Function:  Switch to the selected item in the AES_EBU MENU          */
/*  Remarks:    AES-EBU MENU function                          */
/*          Cursor position on line 2 is updated                */
/*  Returns:    Nothing                                    */
/*  Updates:    Ptr, UcharVal, FuncSave, FunState and FuncTriggers        */
...
void AES_EBUDown(){
  ...
  if ( AES_EBUNdx[AES_EBUnNdx] > AES_EBUSignalItem)
    if ( Flags.NormalLockOn || ( AES_EBUConfig[AES_EBUnNdx].Signal == AES_EBUWordclock))
      return;
  ...
  WriteChar1( '/');
  WriteCode1( AES_EBU_menu[AES_EBUNdx[AES_EBUnNdx]].ItemTxt);

  switch ( AES_EBUNdx[AES_EBUnNdx]) {
    case AES_EBUSignalItem:
      WriteCode1( Select_Txt);
      UcharVal = AES_EBUConfig[AES_EBUnNdx].Signal;
      break;

    case AES_EBULevelItem:
      WriteCode1( Select_Txt);
      UcharVal = AES_EBUConfig[AES_EBUnNdx].Level;
      break;

    case AES_EBUTimingItem:
      WriteCode1( Select_Txt);
      UcharVal = AES_EBUConfig[AES_EBUnNdx].Timing;
      break;
    }
  ...
  Ptr = &AES_EBU_submenu[AES_EBUNdx[AES_EBUnNdx]];
...
void AES_EBUSignalSelect(){
  ...
  if ( FuncSave || FuncEscape) {
    if ( FuncSave) {                // If audio signal has changed..
      ...
      AES_EBUConfig[AES_EBUnNdx].Signal = UcharVal;
      }
    else {                      // ..restore audio signal
      if ( UcharVal != AES_EBUConfig[AES_EBUnNdx].Signal)
        if ( ErrorCode = AES_EBUUnitDrv( AES_EBUnNdx, AES_EBUConfig[AES_EBUnNdx].Signal,
             AES_EBUConfig[AES_EBUnNdx].Level, AES_EBUConfig[AES_EBUnNdx].Timing)) {
          GotoErrorStatus( ErrorCode);
          return;
          }
      }
  ...
  if ( FuncTriggers & 0x03) {            // If UP/DOWN button pressed..
    if ( FuncTriggers & 0x01) {          // If button pressed is UP..
      if ( UcharVal-- == AES_EBUStereo800Hz)
        UcharVal = AES_EBUWordclock;
      }
    else                          // ..button pressed is DOWN
      if ( UcharVal++ == AES_EBUWordclock)
        UcharVal = AES_EBUStereo800Hz;

                                // If actual signal
    if ( UcharVal == AES_EBUConfig[AES_EBUnNdx].Signal)
      FuncState = 1;                  //  remove SAVE option
                                // Transmit audio signal
    if ( ErrorCode = AES_EBUUnitDrv( AES_EBUnNdx, UcharVal, AES_EBUConfig[AES_EBUnNdx].Level, AES_EBUConfig[AES_EBUnNdx].Timing)) {
      GotoErrorStatus( ErrorCode);
      return;
      }
    }
...
void AES_EBULevelSelect(){
  ...
  if ( FuncSave || FuncEscape) {
    if ( FuncSave) {                // If audio level has changed..
      ...
      AES_EBUConfig[AES_EBUnNdx].Level = UcharVal;
      }
    else {                      // ..restore audio level
      if ( UcharVal != AES_EBUConfig[AES_EBUnNdx].Level)
        if ( ErrorCode = AES_EBUUnitDrv( AES_EBUnNdx, AES_EBUConfig[AES_EBUnNdx].Signal,
            AES_EBUConfig[AES_EBUnNdx].Level, AES_EBUConfig[AES_EBUnNdx].Timing)) {
          GotoErrorStatus( ErrorCode);
          return;
  ...
  if ( FuncTriggers & 0x03) {            // If UP/DOWN button pressed..
    if ( FuncTriggers & 0x01) {          // If button pressed is UP..
      if ( UcharVal-- == AES_EBUSilence)
        UcharVal = AES_EBULevel20dB;
    ...
    if ( UcharVal == AES_EBUConfig[AES_EBUnNdx].Level)
      FuncState = 1;                  //  remove SAVE option
                                // Transmit audio level
    if ( ErrorCode = AES_EBUUnitDrv( AES_EBUnNdx, AES_EBUConfig[AES_EBUnNdx].Signal, UcharVal, AES_EBUConfig[AES_EBUnNdx].Timing)) {
      GotoErrorStatus( ErrorCode);
      return;
      }
    }
...
void AES_EBUTimingSelect(){
  ...
  if ( FuncSave || FuncEscape) {
    if ( FuncSave) {                // If timing has changed..
      ActivePresetNo = 0;            // Clear possible preset
                              ...
                              // Store new audio timing
      AES_EBUConfig[AES_EBUnNdx].Timing = UcharVal;
      }
    else {                      // ..restore AES/EBU timing
      if ( UcharVal != AES_EBUConfig[AES_EBUnNdx].Timing)
        if ( ErrorCode = AES_EBUUnitDrv( AES_EBUnNdx, AES_EBUConfig[AES_EBUnNdx].Signal,
                    AES_EBUConfig[AES_EBUnNdx].Level, AES_EBUConfig[AES_EBUnNdx].Timing)) {
          GotoErrorStatus( ErrorCode);
          return;
  ...
  if ( FuncTriggers & 0x03) {            // If UP/DOWN button pressed..
    if ( FuncTriggers & 0x01){            // If button pressed is UP..
      if ( UcharVal-- == AES_EBUPAL)
        UcharVal = AES_EBUNTSCPhase5;
      }
    else                          // ..button pressed is DOWN
      if ( UcharVal++ == AES_EBUNTSCPhase5)
        UcharVal = AES_EBUPAL;

                                // If actual AES/EBU timing..
    if ( UcharVal == AES_EBUConfig[AES_EBUnNdx].Timing)
      FuncState = 1;                  //  remove SAVE option
                                // Transmit AES/EBU timing
    if ( ErrorCode = AES_EBUUnitDrv( AES_EBUnNdx, AES_EBUConfig[AES_EBUnNdx].Signal, AES_EBUConfig[AES_EBUnNdx].Level, UcharVal)) {
      GotoErrorStatus( ErrorCode);
      return;
      }
    }
...
void GenlockTimeSelect(){
  ...
  if ( UcharVal == SDIGenlock) {
    ...
    SDITimeAdjust( 2, tmpSystem, GenlockConfig[UcharVal].Delay);
    }
  else {
    ...
    AnalogTimeAdjust( 2, tmpSystem, GenlockConfig[UcharVal].Delay);
    }
...
void PresetNameSelect(){
  ...
  if ( FuncTriggers & 0x0C) {        // If UP/DOWN button pressed..
    ...
    TextBuffer[UcharVal1] =\
       FindNextASCII(( FuncTriggers & 0x04), FALSE, TextBuffer[UcharVal1]);
...
void ConfigLockSelect(){
  ...
  WriteCodeLN2( 6, LockArrTxt[UcharVal]);
  WriteCodeLN2( 21, OffOnArrTxt[ UcharVal2]);

  switch ( FuncState) {
    case 1:
    case 5:                        // Mark selected item
      WriteItemArrows( 6, Cstrlen( LockArrTxt[UcharVal]));
      break;
        ...
        switch( IntVal) {
          ...
          case 1:                  // Options test 1:
            switch ( Connectors[UcharVal].HWType) {
              ...
              case PT8635:          // AES/EBU audio generator
...
void DiagnoseKeyboardTest(){
  ...
  if ( Button_Command != NO_Button)
    ButtonPressed = Button_Command;
  else
    ButtonPressed = FALSE;
...
void DiagnoseConfigTest(){
  ...
  switch ( UcharVal) {
    case MainConn:
      strcpy( TextBuffer, "PT5210");
      break;

    case OSCConn:
      if ( Int2GenlockUnit.Present == TRUE) {
        strcpy( TextBuffer, "PT8610");
        break;
        }                        // Break IS missing!!
...
void DiagnoseErrorQueueReset(){
  ...
  if ( FuncSave || FuncEscape) {
    ...
    if ( FuncSave) {
      ...
      LevelStatus = 0x7FFF;      // Reset level error, (1=no error)
