...
********************************************************************
HISTORY:
990824 Correction for faulty NV-RAM's (autoincrement faiL)
981112	RELEASED 3.1
980915	RELEASED 3.0
...
*/
// IF not option then remove the next line
#define OPTION
...
{  UC k1, k2, k3, k4;
   ...
   // GET TRIM-POT VALUES
   Wr_iic(ram_addr, daclev1_addr);		// set RAM address
   k1= Rd_iic(ram_addr);	// transfer value
   k2= Rd_iic(ram_addr);	// there is auto-increment
   k3= Rd_iic(ram_addr);	// transfer value
   k4= Rd_iic(ram_addr);

   Wr_iic2(dac1_addr, 0, k1);
   Wr_iic2(dac1_addr, 1, k2);
   Wr_iic2(dac2_addr, 0, k3);
   Wr_iic2(dac2_addr, 1, k4);
/*
   Wr_iic(ram_addr, daclev1_addr);		// set RAM address
...
*/
   ...
   Wr_iic(ram_addr, daclev1_addr+4);		// set RAM address
