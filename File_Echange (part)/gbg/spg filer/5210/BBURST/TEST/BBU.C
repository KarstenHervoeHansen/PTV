...
UL Rd_iic_long(UC adr)
{  UL x, y;
   UC i;
   x= 0; tmp= 0;		// read H-phase
   for (i=0;i<4;i++)
   {
      Wr_iic(ram_addr, adr++);		// set RAM address
...
{  if (req)
   {//  Wr_iic(ram_addr, trimphag1_addr);	// set RAM address
      return_long_int(Rd_iic_long(trimphag1_addr), Rd_iic_int(trimphag1_addr+4));
   }
   else if (save)
   {  save= 0;
...
{  if (req)
   {  Wr_iic(ram_addr, trimphag2_addr);	// set RAM address
      return_long_int(Rd_iic_long(trimphag2_addr), Rd_iic_int(trimphag2_addr+4));
   }
   else if (save)
   {  save= 0;
...
{  if (req)
   {  req= 0;
      ...
      return_long_int(Rd_iic_long(trimpham1_addr), Rd_iic_int(trimpham1_addr+4));
   }
   else if (save)
   {  save= 0;
...
{  if (req)
   {  Wr_iic(ram_addr, trimpham2_addr);	// set RAM address
      return_long_int(Rd_iic_long(trimpham2_addr), Rd_iic_int(trimpham2_addr+4));
   }
   else if (save)
   {  save= 0;
...
void opstart(void)
{
   ...
   Wr_iic(ram_addr, daclev1_addr+1);		// set RAM address
   Wr_iic2(dac1_addr, 1, Rd_iic(ram_addr));	// there is auto-increment
   Wr_iic(ram_addr, daclev2_addr);		// set RAM address
   Wr_iic2(dac2_addr, 0, Rd_iic(ram_addr));	// transfer value
   Wr_iic(ram_addr, daclev2_addr+1);		// set RAM address
   Wr_iic2(dac2_addr, 1, Rd_iic(ram_addr));

   // GET H-PHASE AND SCH-PHASE CORRECTION VALUES
//   Wr_iic(ram_addr, trimphag1_addr);		// set RAM address
   hcalg1= Rd_iic_long(trimphag1_addr);			// read h-phase
//   Wr_iic(ram_addr, trimphag1_addr+4);		// set RAM address
   schcalg1= Rd_iic_int(trimphag1_addr+4);	// read sch-phase

//   Wr_iic(ram_addr, trimphag2_addr);		// set RAM address
   hcalg2= Rd_iic_long(trimphag2_addr);
//   Wr_iic(ram_addr, trimphag2_addr+4);		// set RAM address
   schcalg2= Rd_iic_int(trimphag2_addr+4);

//   Wr_iic(ram_addr, trimpham1_addr);		// set RAM address
   hcalm1= Rd_iic_long(trimpham1_addr);
//   Wr_iic(ram_addr, trimpham1_addr+4);		// set RAM address
   schcalm1= Rd_iic_int(trimpham1_addr+4);
//   Wr_iic(ram_addr, trimpham2_addr);		// set RAM address
   hcalm2= Rd_iic_long(trimpham2_addr);
//   Wr_iic(ram_addr, trimpham2_addr+4);		// set RAM address
   schcalm2= Rd_iic_int(trimpham2_addr+4);
