...
//   RTL1= -48;	// reload value for Timer 1, sets Baud rate to 9600
   RTL1= -24;	// reload value for Timer 1, sets Baud rate to 19200
   TH0= -255;	// start value
...
//   SndChar(default_addr,"SW",bbu1_addr); // DUMMY CMD to wake unit
   ...
   if (IIC1_Write(multibb_addr,0xff))
   {  options |= 8;	// PT8604
   }
   ...
   if (SndRequest(sync_addr,"GS")==0)
      if (AnswerCheck()==0)
      {  if (((UC)AnswerBuf[0]*100 + (UC)AnswerBuf[1]*10 + (UC)AnswerBuf[2]) & 64)
	   options |= 0x4000;  	// Oven unit
      }
// ******************************************************************
 // RECALL OLD SETTINGS
   ...
   {  while(Timer_flag == 0)	// while waiting for timer flag
	...
	// if (!bufbusy)  // read new orders only when GEN. buffer is empty
         while ((cc=get_char1()) != 0)	// handle characters from buffer
		   put_char0(cc);	// TEST -out to RS232
		//  CharHandler(cc);
