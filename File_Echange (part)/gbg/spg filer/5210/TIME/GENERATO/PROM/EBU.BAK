...
UI filend;
UI n, frame;	   // ›nsket max. antal samples pr. bid
...
/* *************************************************************** */
void tidset(UC samp)	// s‘t antal samples pr. bid
...
{ // char *filnavn= "ind.wav";
   ...
   long *bufptr, nyleng;        // til udregning af fill‘ngde
   UC ste, wide;		// mono/stereo og 8/16 bit
   UI lest;                     // antal byte l‘st fra input
   frame= 1600;
...
*/
   ...
   frl= 255; frh= 0; secl= 255; sech= 0;	// set ur
   minl= 0; minh= 0; hrl= 0; hrh= 0;
...
*/
 //	if (frh>=2 && frl>=5)		// PAL-G TIME CODE
	if (n%25==1)
	{  frh= 0; frl= 0; secl += 1;
	   // printf("\n");
	}
...
*/
	...
	{ if (frame<3)
	 {  c= ((secl&1)+(a&1)) & 1;
	    if ((sech&1)^((secl&2)>>1)^(b&1)^((a&2)>>1)^c ==0)
	    {   a+= 1;
		frl+= 1;
	       if (a==10)
	       {  b++; frh++;
	 ...
	 {  c= ((secl&1)+(a&1)) & 1;
	    if ((sech&1)^((secl&2)>>1)^(b&1)^((a&2)>>1)^c)
	    {   a+= 1;
		frl+= 1;
	       if (a==10)
	       {  b++; frh++;
