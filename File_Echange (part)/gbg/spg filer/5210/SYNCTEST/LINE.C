...
{  UI i;
   ...
   if (genmode>=2 && genmode<=5)  goto adown;
...
// ADOWN  :
   ...
   {  btel -= 1;		// count 3xlines
     if (btel==0)
	// 1 frame gone
      {  if (genmode>=2 && genmode<=5)  goto seta;
       	 if (syncav)
...
// get new duration and line type from table **
   ...
   {  cvartel -= 1;	// count duration (c= c-1)
	 ...
	 linadd= 0;
	 if (g_sel)  subcref += subcinc_g; // add 25Hz offset to subc
	 ...
	 if (genmode>=2 && genmode<=5)  doflags |= btable_10[btel];
...
{  if (g_sel)
   {  subcpot= (((UL)genphase_g * 1135 / hlen_g) % 1024) << 6;
      ...
      if (href >= ((long)hlen_g<<8))  href= href - ((long)hlen_g<<8);
   }
   else
   {  subcpot= (((UL)genphase_m * 910 / hlen_m) % 1024) << 6;
      href= hphzero_m + (UL)genphase_m;
      if (href >= ((long)hlen_m<<8))  href= href - ((long)hlen_m<<8);
   }
   domode= 0;
}
// ***************************************************************
