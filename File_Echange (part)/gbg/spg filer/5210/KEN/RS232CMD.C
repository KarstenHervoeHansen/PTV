...
void Audio() {
	...
	if ( AES_EBUUnit.Present) {
		...
		if ( CmdExecute && !SCPIError) {
			if ( CmdRequest) {
				Udr_CPutNChar( AES_EBUSignalPar[AES_EBUConfig.Signal].LongName);
				Udr_PutChar( ',');

				Udr_CPutNChar( AES_EBULevelPar[AES_EBUConfig.Level].LongName);
				Udr_PutChar( ',');

				Udr_CPutNChar( AES_EBUTimingPar[AES_EBUConfig.Timing].LongName);
				Udr_PutNewline();
			}
			else
				SCPIError = SyntaxErr;
