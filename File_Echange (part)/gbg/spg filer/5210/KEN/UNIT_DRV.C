...
UC TestAES_EBUUnit( void) {
	...
	if ( AES_EBUConfig.Signal == AES_EBU48kHz)
...
UC TestAnalogSignalUnit( void) {
	...
	if ( tmp || error)
		return( 1);
												...
												// Test text enable
	tmp = AnalogConfig.TextOn - ( UC) RecInt( AnlSig_addr, "AF", 0xFF, &error);
	if ( tmp || error)
		return( 1);
...
UC TestSDISignalUnit( void) {
	...
	if ( tmp || error)
		return( 1);
												...
												// Test text enable
	tmp = SDIConfig.TextOn - ( UC) RecInt( SDISig_addr, "SF", 0xFF, &error);
	if ( tmp || error)
		return( 1);
