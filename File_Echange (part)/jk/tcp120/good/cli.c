...
// Notes:  This program works as Client, and interacts with
//         Server runing on a PC. 
...
void main(void)
  ...
  while(1)
    ...
    // Run the TCP example
#if 1
    ip_dest_addr[0] = 192; // IP Address of the computer running server
    ip_dest_addr[1] = 168;
    ip_dest_addr[2] = 120;
    ip_dest_addr[3] = 120;
#endif
...
/******************************************************************/
/* NAME       void TCP(void)                                      */
/* PURPOSE    To implement simple Client. Echo data incoming from */
/*            a Server.                                           */
/*                                                                */
/* ARGUMENTS  None                                                */
/*                                                                */
...
/*            If data is received, it is sent back to Client and  */
/*            echoed on terminal.                                 */
/*            mn_recv() is never run, data is received            */
/*            each time mn_send() is run, i.e. together with ACK  */
/*            This function sends data to remote server first     */
/******************************************************************/
void TCP(void)
  {
  SCHAR        socket_no;
  ...
  while(1)
    ...
    /* mn_send sends the data and waits for an ACK. Some echo servers will
     ...
     are (10.12.2007). */
    /* NOTE: data_ptr is a local variable (resides in stack) so it is different */
    /*       from PTR_NULL                                                      */
    ...
    if (data_ptr != PTR_NULL)
      ...
      if (status > 0 && socket_ptr->recv_len > 0)      /* case 1: if in the same packet.... */
        ...
        data_ptr[data_len] = 0;
        sprintf(tbuff, "Same: %s l=%d ", data_ptr, data_len); 
        print(tbuff);
    ...
    do
      ...
      } while (status == NEED_TO_LISTEN);                     /* will never be run here...             */
