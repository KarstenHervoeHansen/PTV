	...
	if ( !sending) {
   	...
   	if (( c != ',') || readbin) {
   		*rgbptr[rsno] = c;					/* Read char from RS232 to string */
			...
			if (( c == '\15') && !readbin) {			/* Check end of message */
				sendok = 1;
...
main( void) {
	...
	if ( !( Poweron_Button & 0x41)) {		/* If "ON" button pressed */
		XBYTE[0x1FFF] = 0;						/*  then clear power-off flag and */
		contrast_port = 0x80;					/*  bootstrap power-on */
	}
	else {
		if ( XBYTE[0x1FFF] == power_down) {	/* If wanted power_down */
	...
	if ( install_head()) {		/* Initialize measuring head */
		head_present = 1;
		errorno = 0;
	}
	else {							/* If head not responding, set defaults */
		initdisplay();
	...
	while( 1) {											/* Main loop */
		if ( !*freeze) {
			...
			if ( !sendok) {							/* Time-out, answer missing */
				if ( !errorno) {
					outtextxy( 0, 16, "Sensor err.    ");
					errorno = 1;
				}
		...
		if ( menu_exit)	{					/* If time-out for menu */
