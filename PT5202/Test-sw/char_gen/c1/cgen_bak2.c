...
enum { X_Sample = 120, X_dotsize = 4, X_with = 5, Y_high = 7 };
...
void Save_line(int line, unsigned char *Data)
{
    ...
    int y_gain;
    for(y_gain = 0; y_gain < X_dotsize; y_gain++)
    {
       ...
       for(x = 0; x < X_Sample; x++, x_pos += XY_GAIN)
       {
           ...
           fillellipse(x_pos, 3*XY_GAIN + XY_GAIN*(line*X_dotsize+y_gain), XY_GAIN / 2, XY_GAIN / 2);
       }
    }
    //if(line == (Y_high - 1))
...
void Filter_line(unsigned char *Data)
{
    ...
    int              Data_delay      [file_lengch] = {  0,  0,    0,   0,  0 };
    //static const int Data_tap_factors[file_lengch] = { 40, 45,  100,  45, 40 };
    //enum { file_scale = 230 };
    static const int Data_tap_factors[file_lengch] = { 20, 30,  60,  30, 20 };
    enum { file_scale = 160 };

    for(x = 0, t_Data = Data; x < X_Sample; x++, t_Data++ )
    {
        long int sum = 0;
        Data_delay[x % file_lengch] = *t_Data;

        for(i = 0, sum = 0; i < file_lengch; i++)
        {
            sum += (long)Data_delay[i] * (long)Data_tap_factors[i];
        }
        if(sum < 0)
               sum = 0;
        *t_Data = sum / file_scale;
    }
}
