...
/* TASKS:                                                                   */
/*                                                                          */
/* NOTES:  20.11.01 - FactGenlockGenUOffDAC() FUNCTION ADDED: JK            */
/*                                                                          */
/*        Instead of FactBBGenDAC we now have two functions:                */
/*        FactBBGenDACG and FactBBGenDACM (ver 5.3)                         */
/*        JK 14.06.2002                                                     */
/****************************************************************************/
/*
...
UC code MasterSWVersion                 = 50;

char code SystemVersionTxt[]            = "1999.0";
...
char code TextInsertCmdTxt[]       = "TEXTINSERT";
char code TextCmdTxt[]             = "TEXT";

char code TextMovementCmdTxt[]     = "MOVEMENT";
char code TextMovTxt[]             = "MOV";
...
char code DACCGmdTxt[]           = "DACG";       // JK 
char code DACCMmdTxt[]           = "DACM";       // JK
...
char code UDACOFFCmdTxt[]                = "DOFF";                  //JK
char code UDACOFFSETCmdTxt[]             = "DOFFSET";               //JK
...
int  code aes_timing[26] = // These are valid AES timings
  {
  -96,
  -88,
  -80,
...
void genlock_adjust(UC);
void calibrate_on(void);
void calibrate_off(void);
void calibrate_store(void);
...
void TSGenTextSignal(void);     // Text commands
void TSGenTextMovement(void);
...
void FactGenlockGenUOffDAC( void); // JK
...
void FactBBGenDACG(void);          // JK
void FactBBGenDACM(void);
...
/* Adding text  command */
#if 1
struct CMD_STRUCT code TSGenLevel1[7] =
  {
  ...
  { EMBAUDIOCmdTxt,   EMBCmdTxt,  &TSGenLevel1[5], &TSGenLevel1_1[0], NONE, TSGenEmbAudioSignal},
  { TextInsertCmdTxt, TextCmdTxt, &TSGenLevel1[6], NULL,              NONE, TSGenTextSignal},
  { TextMovementCmdTxt, TextMovTxt, NULL,          NULL,              NONE, TSGenTextMovement}
  };
...
// JK
struct CMD_STRUCT code FactoryLevel3[7] =
  {
  ...
  { PHASECmdTxt,      PHASCmdTxt, &FactoryLevel3[6], NULL,                NONE, FactGenlockGenPhase},
  { UDACOFFSETCmdTxt, UDACOFFCmdTxt, NULL,           NULL,                NONE, FactGenlockGenUOffDAC},
};
...
// JK
struct CMD_STRUCT code FactoryLevel4[7] =
  {
    ...
    { DACCGmdTxt,      DACCGmdTxt, &FactoryLevel4[6], NULL, NONE, FactBBGenDACG  },
    { DACCMmdTxt,      DACCMmdTxt,  NULL,             NULL, NONE, FactBBGenDACM  },
  };
...
struct PAR_STRUCT code SystemPar[4] = {
        { PALParTxt,     PALParTxt, &SystemPar[1]},
        { PAL_IDParTxt, PAL_IDParTxt, &SystemPar[2]},
        { NTSCParTxt,    NTSCParTxt, &SystemPar[3]},
        { JNTSCParTxt,  JNTSCParTxt,     NULL},
};
...
extern char  PresetNames[4][MaxPresetNameLength];
      ...
      if ((ParCnt != 2) || ((sscanf(Cmd, "%d,%17s", &Reg, Name)) != 2))
        ...
        if (strlen(Name) == MaxPresetNameLength)
          ...
          if ((Reg == 0) || (Reg > NoOfPresets))
            ...
            strcpy(PresetNames[Reg-1], Name);
            //memcpy(&OrgPresetSettings[Reg-1], &Preset, sizeof(PresetObject));
            }
          }
        }
      }
...
void SystemPresetAuthor( void)
  ...
  if ( CmdExecute && !SCPIError)
    ...
    if ( CmdRequest)
      ...
      if (( ParCnt != 2) || (( sscanf( Cmd, "%d,%17s", &Reg, Author)) != 2))
        SCPIError = SyntaxErr;
      else
        {
...
void SystemPresetDate( void)
  ...
  if ( CmdExecute && !SCPIError)
    {
    if ( CmdRequest) /* SYST:PRES:DATE? 4 */
      {
      if (( ParCnt != 1) || ( sscanf( Cmd, "%d", &Reg) != 1))
        SCPIError = SyntaxErr;
      else
        ...
        if (( Reg == 0) || ( Reg > NoOfPresets))
          ...
          /* modifiers 2d below are mine, we want to have 2 digits always */
    ...
    else  /* SYST:PRES:DATE 4 78 05 05 */
      {
      if (( ParCnt != 4) || (( sscanf( Cmd, "%d,%d,%d,%d", &Reg, &year, &month, &day)) != 4))
        ...
        if ( ValidateDate( year, month, day) == FAIL)
          ...
          if (( Reg == 0) || ( Reg > NoOfPresets))
            ...
            {
            NV_Recall( &Preset, PresetPtr[Reg-1], sizeof( Preset));
            /* cast below is mine: JK, otherwise we get overflow */
            Preset.Date = (UL)day + 100 * (UL)month + 10000 * (UL)year;
            NV_Store( &Preset, PresetPtr[Reg-1], sizeof( Preset));
            //memcpy(&OrgPresetSettings[Reg-1], &Preset, sizeof(PresetObject));
            }
          }                               
      ...
      if (ParCnt != 1)
        ...
        tmp = FindParameter(Cmd, OffOnPar);
          ...
          if (tmp == 0)
            ...
            {
            sio0SndCommand(12, SPG_V24_address, "GM0"); // Force genlock internal mode
                                              // Force BB1 PAL mode
  ...
  if ( CmdExecute && !SCPIError)
    ...
    if ( CmdRequest)
      ...
      if ( ParCnt == 0)
        ...
        {
        tmp = FindParameter( Cmd, MinMaxPar); // see if it was MIN/MAX
        if ( tmp > -1)
          {
          if ( tmp == 0)
            genlock_adjust(CMAX);
          ...
          {
          tmp = FindParameter( Cmd, UpDownPar); // see if it was UP or DOWN
          if ( tmp > -1)
            {
            if ( tmp == 0)
              {
...
void calibrate_on(void)
  {                     // if ON, enable calibration
  sio0SndCommand( 12, SPG_V24_address, "GM0"); // set mode: Internal
                                               // set G-system and phase
  sio0SndCommand( 20, BB_V24_address, "HG%ld;", BBSetup[BBOutput1].Delay);
  BBControlPort = 1;                           // Set BB1 to F45MHz
  GenlockCalValue = Calibration.GenlockCalibration.DACUserValue;
                                               // set osc.freq. DAC
...
/**************************************************************************/
/* TSGenTextSignal                                            RS232CMD.C  */
/*                                                                        */
/* Author:    JK                                                          */
/* Revised:   060203                                                      */
/*                                                                        */
/* Function:  Handle TSG text command                                     */
/* Remarks:   When querry:                                                */
/*            :OUTP:TSG:TEXT?;                                            */
/*            the answer is:                                              */
/*            ON,DK-AUDIO: PT5202  i.e. <on | off>,<text_string>          */
/*            To set:                                                     */
/*            :OUTP:TSG:TEXT:ON; or :OUTP:TSG:TEXT monroe;                */
/*            i.e. :OUTP:TSG:TEXT <on | off>;                             */
/*            or   :OUTP:TSG:TEXT <text_string>;                          */
/* Returns:   Nothing                                                     */
/* Updates:    --                                                         */
/**************************************************************************/
void TSGenTextSignal(void)
  {
  ...
  if (CmdExecute && !SCPIError)
    ...
    if (CmdRequest)
      ...
      if (ParCnt)
        ...
        {
        if (CBSetup.TextData.TextEnable)
          rs232puts("ON,");
        else
          rs232puts("OFF,");
        }
      rs232puts(CBSetup.TextData.Text);
      uart1_putchar('\n');
      }
    else
      {                          // if not querry
      if (ParCnt != 1)
        ...
        if (!strcmp(Cmd, "ON"))
          {
          SetCBTextOnoff(ON);
          //SetCBText(CBSetup.TextData.Text);
          }
        else
          if (!strcmp(Cmd, "OFF"))
            {
          ...
          else // must be some text // Max. 16 characters in 5202
            {
            //sscanf(Cmd, "%17s", Text);
            if (strlen(Cmd) > 16)
              SCPIError = InvalidStringData;
            else
              SetCBText(Cmd);
            }
        }
...
/**************************************************************************/
/* TSGenTextMovement                                          RS232CMD.C  */
/*                                                                        */
/* Author:    JK                                                          */
/* Revised:   070203                                                      */
/*                                                                        */
/* Function:  Handle TSG text movement command                            */
/* Remarks:   When querry:                                                */
/*            :OUTP:TSG:MOVEMENT?;                                        */
/*            the answer is:                                              */
/*            ON  i.e. <on | off>                                         */
/*            To set:                                                     */
/*            :OUTP:TSG:MOVEMENT:ON; or :OUTP:TSG:MOV OFF;                */
/*            i.e. :OUTP:TSG:MOV <on | off>;                              */
/* Returns:   Nothing                                                     */
/* Updates:    --                                                         */
/**************************************************************************/
void TSGenTextMovement(void)
  ...
  if (CmdExecute && !SCPIError)
    ...
    if (CmdRequest)
      ...
      if (ParCnt)
        ...
        {
        if (CBSetup.TextData.TextMovement)
          rs232puts("ON");
        else
          rs232puts("OFF");
        }
...
void store_system(int which)
  {

  Calibration.ResetSystem = which;
  NV_Store( &Calibration, CalibrationPtr, sizeof( Calibration));
  ResetInstrument();
  }
...
/* This function implements command:
   :FACT:GENL:DOFFSET <UI>
   :FACT:GENL:DOFFSET?
   which has the purpose of getting V74 DAC offset compensation value
   and storing it in NOVRAM.  This value is used as signed int, but is 
   input and output here as unsigned int.
*/   
void FactGenlockGenUOffDAC( void) // JK
  {
  char buffer[20];
  ...
  if ( CmdExecute && !SCPIError)
    ...
    if ( CmdRequest)
      ...
      if (( ParCnt != 1) || (( sscanf( Cmd, "%u", &DACValue)) != 1))
        ...
        {
        Calibration.GenlockCalibration.DACOffset = DACValue;
        NV_Store( &Calibration, CalibrationPtr, sizeof( Calibration));
        }
      }
    }
...
/* Revised:    000622                                                     */
/*                                                                        */
/* Function:  --  :FACT:GENL:HPHZ?; and :FACT:GENL:HPHZ A,B;              */
/* Remarks:    --                                                          */
/* Returns:    --                                                          */
/* Updates:    --                                                          */
/**************************************************************************/
...
/* Revised:    000622                                                     */
/*                                                                        */
/* Function:  --   :FACT:GENL:PHAS?; and :FACT:GENL:PHAS A,B;             */
/* Remarks:    --                                                         */
/* Returns:    --                                                         */
/* Updates:    --                                                         */
/**************************************************************************/
...
/* Remarks:    --                                                          */
/* Returns:    --                                                          */
/* Updates:    -- Two functions introduced one for Pal one for NTSC        */
/**************************************************************************/
void FactBBGenDACG(void)
  {
  char buffer[30];
  ...
  if (CmdExecute && !SCPIError)
    ...
    if (CmdRequest)
      ...
      if (ParCnt)
        ...
        {
        sprintf(buffer, "%u,%u,%u\n", Calibration.BBCalibration[Suffix[0]].DACGainG, 
                                      Calibration.BBCalibration[Suffix[0]].DACOffsetG,
                                      Calibration.BBCalibration[Suffix[0]].DACLevelG);
        rs232puts(buffer);
        }
      }
    else
      ...
      if (!PasswordProtected)
        ...
        if ((ParCnt != 3) || ((sscanf(Cmd, "%u,%u,%u", &DACGain, &DACOffset, &DACLevel)) != 3))
          ...
          {
          Calibration.BBCalibration[Suffix[0]].DACGainG   = DACGain;
          Calibration.BBCalibration[Suffix[0]].DACOffsetG = DACOffset;
          Calibration.BBCalibration[Suffix[0]].DACLevelG  = DACLevel;  
          NV_Store(&Calibration, CalibrationPtr, sizeof(Calibration));
          }
        }
      }
  ...
  if (CmdExecute && !SCPIError)
    ...
    if (CmdRequest)
      ...
      if (ParCnt)
        ...
        {
        sprintf(buffer, "%u,%u,%u\n", Calibration.BBCalibration[Suffix[0]].DACGainM, 
                                      Calibration.BBCalibration[Suffix[0]].DACOffsetM,
                                      Calibration.BBCalibration[Suffix[0]].DACLevelM);
        rs232puts(buffer);
        }
      }
    else
      ...
      if (!PasswordProtected)
        ...
        if ((ParCnt != 3) || ((sscanf(Cmd, "%u,%u,%u", &DACGain, &DACOffset, &DACLevel)) != 3))
          ...
          {
          Calibration.BBCalibration[Suffix[0]].DACGainM   = DACGain;
          Calibration.BBCalibration[Suffix[0]].DACOffsetM = DACOffset;
          Calibration.BBCalibration[Suffix[0]].DACLevelM  = DACLevel;  
          NV_Store(&Calibration, CalibrationPtr, sizeof(Calibration));
          }
        }
      }
  ...
  if (CmdExecute && !SCPIError)
    ...
    if (CmdRequest)
      ...
      if (ParCnt != 1)
        ...
        sscanf(Cmd, "%d", &test[0]);
        tmp[0] = test[0] & 0x0FF;
        TransmitIIC(0x40, 1, tmp);
        ReceiveIIC(0x40, 1, &tmp[1]);
...
long FLTToSamples(UC system, int F, int L, float T)   {
  ...
  if (system < NTSC_US)                  // If selected system is PAL..
    {
