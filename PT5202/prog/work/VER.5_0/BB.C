...
/* Remarks:                                                                */
/* Returns:                                                                */
/* Updates:  Separate DAC values for PAL and NTSC sent to BB according      */
/*          to BB system. JK 14.06.2002                                   */
/**************************************************************************/
int BBInit(void)
  {
  char cmd;
  ...
  for (i = BBOutput1; i <= BBOutput3; i++)
    ...
    if (Settings.BBSetup[i].System < NTSC_US)
      {
      sio0SndCommand(20, BB_V24_address, "%cE%u;", cmd, Calibration.BBCalibration[i].DACGainG);
      sio0SndCommand(20, BB_V24_address, "%cC%u;", cmd, Calibration.BBCalibration[i].DACOffsetG);
      sio0SndCommand(20, BB_V24_address, "%cD%u;", cmd, Calibration.BBCalibration[i].DACLevelG);
      }
    else
      {
      sio0SndCommand(20, BB_V24_address, "%cE%u;", cmd, Calibration.BBCalibration[i].DACGainM);
      sio0SndCommand(20, BB_V24_address, "%cC%u;", cmd, Calibration.BBCalibration[i].DACOffsetM);
      sio0SndCommand(20, BB_V24_address, "%cD%u;", cmd, Calibration.BBCalibration[i].DACLevelM);
      }
...
/* Returns:                                                                */
/* Updates:  DAC values for PAL or NTSC transmitted to BB                  */
/*          14.06.2002 JK                                                 */
/**************************************************************************/
int SetBBSystem(int output, int system)
  {
  ...
  // Load BB DAC params according to system 
  if (Settings.BBSetup[output].System < NTSC_US)
    {
    sio0SndCommand(20, BB_V24_address, "%cE%u;", cmd, Calibration.BBCalibration[output].DACGainG);
    sio0SndCommand(20, BB_V24_address, "%cC%u;", cmd, Calibration.BBCalibration[output].DACOffsetG);
    sio0SndCommand(20, BB_V24_address, "%cD%u;", cmd, Calibration.BBCalibration[output].DACLevelG);
    }
  else
    {
    sio0SndCommand(20, BB_V24_address, "%cE%u;", cmd, Calibration.BBCalibration[output].DACGainM);
    sio0SndCommand(20, BB_V24_address, "%cC%u;", cmd, Calibration.BBCalibration[output].DACOffsetM);
    sio0SndCommand(20, BB_V24_address, "%cD%u;", cmd, Calibration.BBCalibration[output].DACLevelM);
    }
  return(sio0SndCommand(12, BB_V24_address, "%cH%u;", BBOutputCmd[output], (int) tmp));
  }
