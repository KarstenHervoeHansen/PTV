...
void CBLineInterrupt( void)
  ...
  static UI tmp_t;
   ...
   static UI Restore_Status; 
   static UC StatusPort_Flag;
   static UC TextLineCnt = 75;  //Indicated from which line the Text lines have to be inserted !
...
//Field #1.
  ...
  if ((LineCnt >= TextLineCnt)	&&	//Start of text lines !
      (LineCnt <= (TextLineCnt+8)))
    {
	 if(LineCnt < (TextLineCnt+8))
       {
		 tmp_t = TextLines_SyncLineC[LineCnt - TextLineCnt];
...
//To prevent Interlacing "flicker" Down and Up movements is handled differently.
  ...
  if ((LineCnt >= (TextLineCnt + LineNum)) &&   //263 = number of lines in a feeld #1. !
      (LineCnt <= (TextLineCnt + 8 + LineNum)))
    {
	 if (LineCnt < (TextLineCnt + 8 + LineNum))
       {
		 tmp_t = TextLines_SyncLineD[LineCnt - (TextLineCnt + LineNum)];
    	 tmp_t %= 256;              // extract SramBankX + SyncLineX tmp
    	 tmp_t &= 0x3F;
    	 Status &= ~( S5S4S3S2 | S1S0);
    	 Status |= tmp_t;
