...
enum { TEXTLINES_HIGHT = 18,	// Total number of lines for Text BAR
...
unsigned int VideoFilter(unsigned int InputSample, FiltStruct * FiltDelayLine)
{
    ...
    unsigned int FiltCorf[4] = { 3, 13, 13, 3 }, *FCptr = FiltCorf;
    UC   ReadIndex;

    FiltDelayLine->DelayLine[(FiltDelayLine->InputIndex)++] = InputSample;
    if (FiltDelayLine->InputIndex > 3) FiltDelayLine->InputIndex = 0;
    ReadIndex = FiltDelayLine->InputIndex;

/*
    for (i = 0; i < 4; i++)
    {
        if (ReadIndex > 3) ReadIndex = 0;
        FileSum += *FCptr++ * (FiltDelayLine->DelayLine[ReadIndex ++]);
    }
*/
    FileSum  = (Y = FiltDelayLine->DelayLine[ReadIndex ++]) << 1; // 2
    FileSum += Y; // 1
    if (ReadIndex > 3) ReadIndex = 0;

    FileSum += (Y = FiltDelayLine->DelayLine[ReadIndex ++]) << 3; // 8
    FileSum += Y << 2; // 4
    FileSum += Y; // 1
    if (ReadIndex > 3) ReadIndex = 0;
...
int SRAMVideoTextLoad(void)
{
  ...
  FiltStruct Ufilt = { 0, 0, 0, 0, 0 };
  FiltStruct Vfilt = { 0, 0, 0, 0, 0 };

  Status |= PS;                    // Set PS bit while loading
  Update_StatusPort(Normal_Status = Status);
  ...
  for (l = 0; l < 8; l++)
    ...
    for ( i = 0; i < 96; i++)		// 48 Y sample, 12 U sample & 12 V sample
      {
      if (i & 0x01)
         {
      	if (i & 0x02)
            *VideoDstPtr++ = VideoFilter(*VideoSrcPtr++, &Vfilt);
         else
            *VideoDstPtr++ = VideoFilter(*VideoSrcPtr++, &Ufilt);
         }
      else
          *VideoDstPtr++ = VideoFilter(*VideoSrcPtr++, &Yfilt);
      }
    Str = DispStr;

    for (m = 0; m < 6; m++)		// 6 Y sample, 12 U sample & 12 V sample
      {
      *VideoDstPtr++ = VideoFilter(96, &Yfilt);
      if (i & 0x02)
