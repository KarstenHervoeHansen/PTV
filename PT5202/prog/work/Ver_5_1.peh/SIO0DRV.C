  ...
  do
  {
    ...
    if ( KS_dequeuet( sio0iStream->ID, &tmp, RxTimeout/CLKTICK) == RC_GOOD)
    {
      switch ( tmp)
        ...
        case 0x81:
          return( EventSIO0TxCksumErr); //3403

        default:
          return( EventSIO0Error);  //3400
			}
    }
  } while ( retransmit--);

  return( EventSIO0NoAck);  //3401
}										
...
int sio0SndCommand( int cnt, int addr, const char *format, ...)
{
  ...
  if (( ret == EOF) || ( ret > cnt+16))
  {
    ...
    return( EOF);				   // EOF is -1, the same as OK
  }
/* sio0WaitAcknowledge() returns:
   OK                  = -1
   EventSIO0TxCksumErr = 3403
   EventSIO0Error      = 3400
   EventSIO0NoAck      = 3401
*/   
