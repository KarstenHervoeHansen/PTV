...
enum { TEXTLINES_HIGHT = 21,	// Total number of lines for Text BAR
       TEXTREPEAT_HIGH = 2    // Number of times the text lines shut be repeated
     };
/*
...
void CBLineInterrupt( void)
  ...
  static UI tmp_t;
   ...
   static UC TextRepeat  = 0;	// Counter : display text line
      ...
      (LineCnt <= (TextLineBegin + TEXTLINES_HIGHT)))
    {
	 ...
	 if (LineCnt < (TextLineBegin + TEXTLINES_HIGHT))
       ...
       TextLineCnt = 0;				// prepare for next text view
    	 Status = Restore_Status;	// Restore original pattern
       StatusPort_Flag = TRUE;	// Update Status in hardware
       }
