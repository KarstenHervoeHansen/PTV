...
void RTXCstrupr(char *s);
void RTXCstrlwr(char *s);

#ifdef RTXCIO
extern void detioobj(void);
extern void defiogen(void);
extern void defrtxcio(void);
#endif
...
int nfflag = 1;         /* 1=use near/far, 0=don't user near/far */
...
void main(int argc,char *argv[])
{
   ...
   printf("Embedded System Products Inc. - RTXCgen 04/07/97 Version RTXC v3.2/RTXCio v1.4\n");
      ...
      switch(argv[i][0])
      {
         ...
         case '/': /* DOS switches */
            switch(argv[i][1])
            {
               ...
               case 'm': /* mixed model OFF = no near/far  */
               case 'M':
                  nfflag = 0;
                  break;
...
void n_f(FILE *outf)
{
   if (!nfflag)
   {
      fprintf(outf,"/* undeclare model keywords */\n");
      fprintf(outf,"#define near\n");
      fprintf(outf,"#define far\n");
      fprintf(outf,"#define huge\n");
      fprintf(outf,"#define ks_stk\n\n");
   }
}
...
void RTXCstrupr(char *s)
{
   ...
   for( ; *s; s++)
   {
      c = *s; /* avoid any possible macro side effect */
