...
swap:
      ...
      fprintf(outf,"/* cqueue.c - RTXC Queue definitions - %20s %14s */\n\n",
         ...
         if (konst)
            fprintf(outf,"const ");
	 fprintf(outf,"QUEUE nqueues = NQUEUES;\n\n");
         for (i = 1; i <= nqueues; i++)
         {
            if ( (queue[i].name[0] == ' ') || (queue[i].width == 0) )
               continue;
	    fprintf(outf,"static char ks_queue q%d[Q%dDP][Q%dWD];\n",i,i,i);
         }
         fprintf(outf,"\n");

         if (romqh)
         {
	    fprintf(outf,"QHEADER ks_tblx qheader[1+NQUEUES];\n\n");

            if (konst)
               fprintf(outf,"const ");
	    fprintf(outf,"QKHEADER qkheader[1+NQUEUES] =\n{\n");
