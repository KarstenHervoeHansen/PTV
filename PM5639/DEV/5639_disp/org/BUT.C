...
char take_meas( void) {
	...
	for ( i=0; i<2; i++) {
		...
		while ((!sendok) && GenericTimecount) {	/* Skip current measurement */
...
/*	Syntax:		void learn_whref( char tmp, char maxline);						*/
/*	Remarks:		----																			*/
/*	Returns:		----																			*/
/*	Updates:		----																			*/
/***************************************************************************/
	...
	switch( head_version) {
		case CRThead:
			*barptr[18] = tmp;						/* Selected menu bar */
			...
			if (tmp == maxline - 1)	{				/* if "Enter" selected */
				if (whref_done) {						/* and new wh.ref. is learned */
				...
				if (tmp == 3) {				/* if "Use measurement" selected */
													/* test if overload or low light */
					if ((msm[0]->xm / xfact) < -0.5)		
						show_message( overflow, menu[18].maxline+3);
					else 
						if (msm[0]->yy < 10 / measunit[1].unitfact)
		...
		case PRJhead:
			*barptr[35] = tmp;					/* Selected menu bar */
			if (tmp == 1) {						/* If "Use measurement" selected */
														/* Test if overload or low light */
				if ((msm[0]->xm / xfact) < -0.5)
				...
				if ( !testmode) {
					...
					*barptr[0] = temp->select;
					...
					switch ( temp->select) {
						case 1:
							testmode = 1;
							outtextxy( 1, 3, "Version: "); 	/* show version */
						...
						case 3:										/*  ROM check */
							...
							if ( !sum) {		  /* check sum against figure in ROM */
...
void timetick( void) interrupt 1 {
	...
	if ( butenable)	{						/* skip if buttons not enabled */
		if ( repeat_on)	{					/* sort in repeat_on and combi_on */
			if ( but == oldbut) {
				if (!rep_cnt) {
					butcmd = but;
					newbut =1;
			...
			if ( but != NO_Button) {
				if ( but == oldbut) {
					if (((( but == UP_Button) || (but == DOWN_Button)) &&
						  ( *menuno!=3) && ( *menuno!=16)) ||
						 ( but == ENTER_Button) ||
						 ( but == RGB_Button) ||
						 ( but == CIE_Button) ||
		...
		if ( but == NO_Button) {							/* Not butenable*/
			butenable = 1; 
			oldbut = combi = 0xff; 
			repeat_on = combi_on = 0;
		}	

	if ( non_shdwn)	{		/* if in non shut down (test) */
		laptics++;
   	if ( laptics==40) {
   		laptics = 0;
     		( *lapsec)++;
   	...
   	if ( menu_cnt )	{				/* then test for menu time-out */
   		menu_cnt--;
     		if  ( !menu_cnt) 
				menu_exit = 1;
   	}
   	if ( idle_cnt) {					/* test for main time-out */
   		idle_cnt--;
     		if  ( !idle_cnt ) 
				shut_dwn();
   	}
   	if ( light_on) {				/* test for light time-out */
     		light_cnt--;
     		if  ( !light_cnt) { 
				light_exit = 1; 
				light_on = 0; 
