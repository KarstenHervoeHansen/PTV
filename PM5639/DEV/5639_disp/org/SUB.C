...
void recall_msm( char no) {
	...
	if ( msm[no]->key == 0x55) { 							/* only if not locked */
		...
		if ( msm[no]->mode_no == RGB_mode) { 			/* if in RGB mode */
			...
			*freeze = 1;
			...
			switch ( head_version) {
				case CRThead:
					*barptr[1] = barptr_tmp;
					...
					outtextxy( 16, 15, ftoa( msm[no]->yy * 
						measunit[msm[no]->munit_no].unitfact, decimals));
				...
				case PRJhead:
					*barptr[30] = barptr_tmp;
					...
					rgb_tmp = msm[no]->yy * measunit[msm[no]->munit_no].unitfact*
							cfactor[msm[no]->cfactor_no]->cfactor;
			...
			if (( barptr_tmp < 5) ) {							/* and wh.ref name */
				outtextxy( 4, 16, "Ref: ");
			...
			*freeze = 1;
			...
			if ( msm[no]->yy >= 0.1 / measunit[1].unitfact)
															  /* write deviation in CIELUV */
				outtextxy( 16, 13, ftoa( msm[no]->dev, 0));
...
void initmode() {
	...
	if ( sup[0]->mode_no == xyY_mode) {		/* set up screen for xyY display */
	...
	else {										/* set up screen for RGB display */
		ymax = rgbbar_max;
		ymin = rgbbar_min;
