...
void common_calc(void)
  ...
  float sum, xx, zz, divisor, utmp, vtmp, v_tmp;

  sample_x = sample->x;                      /* save Sensor values */
  sample_y = sample->y;
  sample_z = sample->z;
  ...
  xx = sample_x * k[0] + sample_y * k[3] + sample_z * k[6];   /* = true X */
  zz = sample_x * k[2] + sample_y * k[5] + sample_z * k[8];    /* = true Z */

  msm[0]->yy = yy = sample_x * k[1] + sample_y * k[4] + sample_z * k[7]; /* = true Y */
  ...
  msm[0]->xm = xm = sample_x * xfact;
  msm[0]->ym = ym = sample_y;         /* original line: yfact is always 1.0 */
  msm[0]->zm = zm = sample_z * zfact;   

  lumlow = 0;

  switch (head_version)
    {
    case CRThead:  /* if (yy < 10000) lumlow++; */
      if (yy < (0.1  / measunit[1].unitfact)) 
        lumlow++;
      break;
    case PRJhead:                   /* #define default_cfactor 14.28571429 */
...
unitfact is one of the above (JK)
...
/************************************************************************/
  ...
  xerr = (int)((sample_x + 0.0005)*1000) - (int)((sample_y + 0.0005)*1000);
  yerr = (int)((sample_z + 0.0005)*1000) - (int)((sum + 0.0005)*1000);
}
