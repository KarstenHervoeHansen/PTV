 ...
 * NOTES:           The logo is inserted in upper part of Philips NTSC picture
 *                  Logo file is top.bmp
 *                  Logo starts at 4870   (Start address in PROM)
 */
...
#define BMP_FILE "top.bmp"

#define BLOCK           4870

#define BMP_LINE_LEN    (long)(246 * 3 + 2)   /* 740 orgLine len in .BMP-fil. 246 samples of 3 bytes */
#define NR_BMP_LINES     57    /* Number of lines in our .bmp file */
#define NR_LINES         56    /* Number of logo lines in our picture */
#define CHAN_LINE_LEN  1024L   /*Line len in channelx.dat-files (word) */

#define LOGO_LEN        (int)(196) /* 196 */
#define LOGO_START      286

#define LOGO_LINES_TO_SKIP  2  /* 7 skip first LOGO_LINES_TO_SKIP when loading logo */
...
/* At this point there is a black bar above the logo, which is the rest
   of black field, now partialy covered by logo. We have to cover this 
   black bar with white.  We will use last logo line to get white.
*/
  block = BLOCK - 9; /* move 9 pixels up */

  for (i = 0; i < 9; i++)
    {
    fileload((UL)(block + i));
    for (j = LOGO_START + 13; j < LOGO_START + 180; j++)
      {
      chan1_buff[j] = Ybuff[j - LOGO_START];
      chan2_buff[j] = Ubuff[j - LOGO_START];
      chan3_buff[j] = Vbuff[j - LOGO_START];
