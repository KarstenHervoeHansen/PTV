 ...
 * CODE INSP.
 * TEST
 * LAST UPDT:       15.12.2005
 * NOTES:           This program manipulates standard 8633 picture data
 *                  by inserting IB3 (Spain) logo in the upper white 
 *                  field on Philips 4:3 test picture for PAL.
 *                  Reflection pin and black rectangle in the upper 
 *                  white field are removed first.
 *                  The logo is filtered and resized and is in ib1.bmp
 *                  file.  Logo's background is pure white.
 *                  As the customer wants logo to occupy almost all 
 *                  heigth of the upper white field, this program will
 *                  treat logo's background as transparent, otherwise
 *                  the logo (which is rectangular) will overwrite area
 *                  beyond the Philips circle.
 *                  
 */
...
#define BMP_FILE "ib1.bmp"     /* File containing logo */
#define OUT_FILE "dump.bmp"    /* Output file containing picture with logo */

#define IMLEN    800           /* length of output image */
#define IMHEIGHT 2693          /* height of output image */
#define OUT_IMSTART 230        /* start of image for bmp output file */

#define IMSTART   230          /* Start address of image in .dat files */
                               /* PAL Philips 4x3                      */

#define BLOCK   IMSTART + 39   /* Start adderss in PROM where our logo is to be */
#define NR_LINES          97   /* Number of logo lines in our picture */
#define LOGO_START  (int)295   /* X coordinare of upper left logo sample in our picture */
#define LOGO_LEN         160   /* Length of logo in our picture in samples */
...
UC   tranparent_help[CHAN_LINE_LEN];
...
/* Now we cover the black pin on the left with white.  Upper left is 270, 98,
   len is 26 samples, it has 42 lines.
*/
  block = IMSTART + 56 + 41;  /* First line of black rectangle */
  ...
  for (i = 0; i < NR_LINES; i++)
    {
    fileload((UL)(block + i));
    /* loadlogo() loads data from logo into bmp_buff[] */
    loadlogo((UL)(i + LOGO_LINES_TO_SKIP), bmp_bytes_per_line, bmp_info.biHeight, bmp_header.bfOffBits);
    /* rgb_yuv() fills Ybuff[], Ubuff[] and Vbuff[] with logo YUV */
    rgb_yuv(logo_len);


/**********************************************************/
/* NOTE: Because logo is a rectangle which is too bigg to */
/* fit entirely into upper white rectangle inside circle  */
/* we need to have narrower boundaries when inserting     */
/* upper one/third of logo height.  Logo will be inserted */
/* in two steps, with different boundaries                */
/**********************************************************/
...
/* Now we cover the black pin on the left with white.
   Len is 26 samples, it has 42 lines.
*/
  block = IMSTART + 56 + 41 + 576;  /* First line of black rectangle */
...
/* Now we cover the black pin on the left with white.
   Len is 26 samples, it has 42 lines.
*/
  block = IMSTART + 56 + 41 + 576 + 576;  /* First line of black rectangle */
...
void rgb_yuv(int len_of_logo)
  ...
  for (i = 0; i < (len_of_logo); i++)
   ...
   /* see if it is pure white, if is, then put FF in tranparent_help[i] */
