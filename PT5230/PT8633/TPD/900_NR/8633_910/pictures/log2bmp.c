/* FILE NAME        log2bmp.c
 * PURPOSE          Generic program to make a .bmp file from logX.dat.
 *                  files, used in Moving Logo project
...
#define OUT_FILE "LOGOX.bmp"    /* Output file containing picture */

#define IMLEN     512          /* length of output image 512 */ 
#define IMHEIGHT  44           /* height of output image 44 */
#define IMSTART    0           /* Start address of output image in .dat files */

#define CHAN_LINE_LEN  512   /*Line len in channelx.dat-files (word) in samples */
#define FILTER 1
...
void main(void)
  ...
  printf("LOG2BMP; converts LOGOX.DAT to LOGX.BMP\n");
...
void fileopen(void)
  ...
  if ((chan1_fptr = open("LOGOY.DAT", O_RDONLY | O_BINARY)) == -1)
    {
    puts("Cant open input file.\n");
    exit(1);
    }
  if ((chan2_fptr = open("LOGOU.DAT", O_RDONLY | O_BINARY)) == -1)
    {
    puts("Cant open input file.\n");
    exit(1);
    }
  if ((chan3_fptr = open("LOGOV.DAT", O_RDONLY | O_BINARY)) == -1)
    {
    puts("Cant open input file.\n");
    exit(1);
    }
