 ...
 * CODE INSP.
 * TEST
 * LAST UPDT:       14.01.2005
 * NOTES:           This program manipulates standard 8631 picture data
 ...
 *
 *                  Note that it will also be necessary to modify 8631
 ...
 *
 *                  In standard 8631 picture data, the following lines are
 *                  involved when using windowed patterns and White100
 *                  pattern:
 *
 *                  Window10_G    EQU 064H  Line 100
 *                  Window15_G    EQU 019H  Line  25
 *                  Window20_G    EQU 01AH  Line  26
 *                  Window100_G   EQU 01BH  Line  27 (This will be the same)
 *                  
 *                  Whiteline_G   EQU 014H  Line  20 (used for full screen white White100)
 *
 *                  use following files for segmentation and splitting:
 *                     segmenth FL=1 LL=7958 SL=32
 *
 *                  SEGMENTH EXE        21,200  09-14-01  1:21p
 *                  SPLT5210 EXE         8,400  09-14-01  1:07p
 *
 *                  Checksums:
 *                            V65: 97D6
 *                            V66: FB9C 
...
#define IMLEN    1024          /* length of output image */
...
/*********************************************************/
  ...
  shuffle_data(25);  /* rearrange data for lines 25 (W15)   */
  shuffle_data(26);  /* rearrange data for lines 26 (W20)   */
  shuffle_data(100); /* rearrange data for lines 100 (W10)  */
...
/**********************************************************************/
  ...
  open_output_file();   /* open dump.bmp           */
...
void shuffle_data(UL line_nr)
  ...
  /* go to pixel 599 and copy data for 250 pixels */
  j = 821;

  for (i = 599; i > (599 - 250); i--)
    {
  ...
  /* go to pixel 337 and copy data for 270 pixels */
  j = 115;

  for (i = 337; i < (337 + 270); i++)
    {
    chan1_buff[j] = chan1_buff[i];
    chan2_buff[j] = chan2_buff[i];
    chan3_buff[j] = chan3_buff[i];
